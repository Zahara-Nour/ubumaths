...
/**
 * Variables aléatoires, 1ʳᵉ spé : 2 fiches neuves (2026-09-26)
 * ============================================================
 *
 * Sur le modèle des probabilités conditionnelles
 * (create-probabilites-conditionnelles-1spe.ts), programme de 1re de 2026
 * (variables aléatoires réelles : loi, espérance, variance, écart type,
 * linéarité de l'espérance, König-Huygens ; expérimentations : moyenne
 * d'échantillon, règle des 2σ/√n) :
 *  - « Entraînement technique — Variables aléatoires » : 10 exercices
 *    `automatisme` ;
 *  - « Applications des variables aléatoires » : 9 exercices `application`
 *    en 3 sections.
 * Aucun exercice existant sur le thème : nouveau thème « Variables aléatoires ».
 *
 * Contenu vérifié : calcul (Fraction/sympy, 226 assertions), rendu écran de chaque
 * formule (`pnpm check:ubumark`), PDF fiche + corrigé FR/EN compilés avec le
 * compilateur de prod (typst.ts 0.6.1-rc5), sans débord de colonne (texte et
 * tracés), sans virgule décimale en anglais.
 *
 ...
 *
 * Usage :
 *   pnpm tsx scripts/create-variables-aleatoires-1spe.ts            # simulation
 *   pnpm tsx scripts/create-variables-aleatoires-1spe.ts --publier  # écrit en prod
 */
...
const TOPIC = 'Variables aléatoires';

const DIR =
	'/private/tmp/claude-501/-Users-david-Coding-js-ubumaths/5a4e98b0-c76d-42bf-8e8c-ee835a4273ea/scratchpad/va';
...
async function main() {
	...
	const FICHES = {
		...
		app: {
			title: 'Applications des variables aléatoires',
			description: null,
			translations: { en: { title: 'Random variables in context' } }
		}
	};
	...
	try {
		...
		// Fiche technique : sans sections, dans l'ordre de titles.txt
