...
/**
 * Suites, 1ʳᵉ spé : 24 exercices et 2 fiches (2026-09-24)
 * ======================================================
 *
 * Sur le modèle des fiches « second degré » :
 *  - « Entraînement technique — Suites » : 13 exercices `automatisme`, sans sections ;
 *  - « Applications des suites » : 11 exercices `application` en 3 sections
 ...
 * formel, rendu PDF compilé (fiche + corrigé, FR et EN) sans erreur de formule.
 *
 ...
 *
 * Usage :
 *   pnpm tsx scripts/create-suites-1spe.ts            # simulation
 *   pnpm tsx scripts/create-suites-1spe.ts --publier  # écrit en prod
 */
...
const TOPIC = 'Suites';
const DIR =
	'/private/tmp/claude-501/-Users-david-Coding-js-ubumaths/5a4e98b0-c76d-42bf-8e8c-ee835a4273ea/scratchpad/suites';
...
const SECTIONS_EN: Record<string, string> = {
	s1: 'Using a sequence model',
	s2: 'Building a sequence model',
	s3: 'Investigation'
};
...
async function main() {
	...
	const FICHES = {
		tech: {
			title: 'Entraînement technique — Suites',
			description:
			...
			translations: {
				en: {
					title: 'Technical practice — Sequences',
					description:
		...
		app: {
			title: 'Applications des suites',
			description: null,
			translations: { en: { title: 'Sequences in context' } }
		}
	};
	...
	for (const s of plan.sections) {
		const v = validateCreateWorksheetSection({
			title: s.title,
			position: s.position,
			translations: { en: { title: SECTIONS_EN[s.id] } }
		});
		if (!v.success)
