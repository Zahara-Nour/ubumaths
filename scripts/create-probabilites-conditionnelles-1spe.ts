...
/**
 * Probabilités conditionnelles et indépendance, 1ʳᵉ spé : 2 fiches neuves (2026-09-25)
 * ====================================================================================
 *
 * Sur le modèle du produit scalaire (create-produit-scalaire-1spe.ts), programme de
 * 1re de 2026 (probabilités conditionnelles, indépendance, probabilités totales,
 * épreuves de Bernoulli pour n ⩽ 4, Monte-Carlo) :
 *  - « Entraînement technique — Probabilités conditionnelles » : 10 exercices
 *    `automatisme` ;
 *  - « Applications des probabilités conditionnelles » : 8 exercices `application`
 *    en 3 sections.
 * Aucun exercice existant sur le thème : nouveau thème « Probabilités ».
 *
 * Contenu vérifié : calcul (Fraction/sympy, 232 assertions), rendu écran de chaque
 * formule (`pnpm check:ubumark`), PDF fiche + corrigé FR/EN compilés avec le
 * compilateur de prod (typst.ts 0.6.1-rc5), sans débord de colonne (texte et tracés).
 *
 ...
 *
 * Usage :
 *   pnpm tsx scripts/create-probabilites-conditionnelles-1spe.ts            # simulation
 *   pnpm tsx scripts/create-probabilites-conditionnelles-1spe.ts --publier  # écrit en prod
 */
...
const TOPIC = 'Probabilités';

const DIR =
	'/private/tmp/claude-501/-Users-david-Coding-js-ubumaths/5a4e98b0-c76d-42bf-8e8c-ee835a4273ea/scratchpad/proba';
...
async function main() {
	...
	const FICHES = {
		...
		app: {
			title: 'Applications des probabilités conditionnelles',
			description: null,
			translations: { en: { title: 'Conditional probability in context' } }
		}
	};
