...
/**
 * Fonctions trigonométriques, 1ʳᵉ spé : 20 exercices neufs, 2 fiches (2026-09-25)
 * ==============================================================================
 *
 * Sur le modèle des fiches de dérivation (create-derivation-1spe.ts) :
 *  - « Entraînement technique — Fonctions trigonométriques » : 12 exercices
 *    `automatisme`, sans sections ;
 *  - « Applications des fonctions trigonométriques » : 8 exercices `application`
 *    en 3 sections (cercle et mesures, phénomènes périodiques, recherche).
 * Aucun exercice de 1re sur ce thème n'existait en base (inventaire du 2026-09-24).
 *
 * Contenu vérifié : calcul formel (sympy, assertions), rendu écran de chaque
 * formule (`pnpm check:ubumark`), PDF fiche + corrigé FR/EN compilés sans erreur
 * avec le compilateur de prod (typst.ts 0.6.1-rc5, cercles CeTZ compris).
 *
 ...
 * arrêt) ; chaque écriture doit rendre sa ligne ; en cas d'échec, fiches et
 * exercices créés supprimés.
 *
 * Usage :
 *   pnpm tsx scripts/create-trigonometrie-1spe.ts            # simulation
 *   pnpm tsx scripts/create-trigonometrie-1spe.ts --publier  # écrit en prod
 */
...
const TOPIC = 'Fonctions trigonométriques';
const DIR =
	'/private/tmp/claude-501/-Users-david-Coding-js-ubumaths/5a4e98b0-c76d-42bf-8e8c-ee835a4273ea/scratchpad/trigo';
...
async function main() {
	...
	const FICHES = {
		tech: {
			title: 'Entraînement technique — Fonctions trigonométriques',
			description:
				'Rédiger les calculs et justifier les réponses sur une copie. Donner les valeurs exactes, sans calculatrice sauf indication contraire.',
			translations: {
				en: {
					title: 'Technical practice — Trigonometric functions',
					description:
						'Show your working and justify your answers on paper. Give exact values, without a calculator unless stated otherwise.'
				}
			}
		},
		app: {
			title: 'Applications des fonctions trigonométriques',
			description: null,
			translations: { en: { title: 'Trigonometric functions in context' } }
		}
	};
	...
	try {
		...
		// Fiche technique : sans sections, dans l'ordre des titres
		const ficheTech = await creerFiche(FICHES.tech);
		const ordreTech = tech.map((e) => ids.get(e.key)!);
		await ajouter(
			ordreTech.map((id, i) => ({
		...
		// Fiche d'application : 3 sections, dans l'ordre du plan
