...
/**
 * Produit scalaire, 1ʳᵉ spé : exercices neufs + existants, 2 fiches (2026-09-25)
 * =============================================================================
 *
 * Sur le modèle des fiches de trigonométrie (create-trigonometrie-1spe.ts) :
 *  - « Entraînement technique — Produit scalaire » : exercices `automatisme` neufs,
 *    entrelacés par thème avec les exercices existants de David (fiches « Produit
 *    scalaire » et « Evaluation : Produit scalaire », qu'on ne modifie pas) ;
 *  - « Applications du produit scalaire » : exercices `application` neufs en 3
 *    sections, complétés par les exercices existants du même esprit.
 * Principe de David : les fiches sont les plus complètes possible — elles incluent
 * les exercices existants du thème. Ceux-ci ont déjà corrigé et anglais : on ne
 * les touche pas.
 *
 * Contenu vérifié : calcul (sympy/numérique, assertions), rendu écran de chaque
 * formule (`pnpm check:ubumark`), PDF fiche + corrigé FR/EN compilés avec le
 * compilateur de prod (typst.ts 0.6.1-rc5).
 *
 ...
 *
 * Usage :
 *   pnpm tsx scripts/create-produit-scalaire-1spe.ts            # simulation
 *   pnpm tsx scripts/create-produit-scalaire-1spe.ts --publier  # écrit en prod
 */
...
const EXISTANTS_APRES: Record<string, [string, string][]> = {
	'01': [
		['67743200', 'Calcul de produit scalaire'],
		['a723b0ca', 'Calcul de produit scalaire (2)'],
		['b52aeceb', 'Calcul de produit scalaire (3)']
	],
	'03': [['6a0166d4', 'Calcul de produit scalaire (4)']],
	'06': [['36188a42', 'Orthogonalité']],
	'08': [['98e57f6c', 'Deuxième formule de la Médiane']]
};
...
const EXISTANTS_SECTION: Record<string, [string, string | null][]> = {
	s1: [
		['8547cfc8', 'Triangle rectangle'],
		['96c54dc9', 'Projeté orthogonal'],
		['6dc565e1', null]
	],
	s2: [['d32464db', 'La tyrolienne']],
	s3: [['709bf53b', null]]
};
const DIR =
	'/private/tmp/claude-501/-Users-david-Coding-js-ubumaths/5a4e98b0-c76d-42bf-8e8c-ee835a4273ea/scratchpad/ps';
...
async function main() {
	...
	const FICHES = {
		...
		app: {
			...
			description: null,
			translations: { en: { title: 'Scalar product in context' } }
		}
	};
