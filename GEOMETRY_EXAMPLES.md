...
async function generateHomothetyFigure(container: HTMLElement, ratio: number = 1.5) {
  ...
  app.createMeasurement('area_A_prime_B_prime_C_prime', 'area', ['A_prime', 'B_prime', 'C_prime'], { display: true });
...
async function validateMeasurements(app: MathGraphApp, exercise: GeometryExercise) {
  ...
  for (const [key, expected] of Object.entries(expectedMeasurements)) {
    ...
    if (diff <= expected.tolerance) {
      ...
      feedback.push(`❌ ${expected.description} : ${value.toFixed(1)}${expected.unit} (attendu : ${expected.value}${expected.unit} ±${expected.tolerance})`);
    }
  }
...
async function validateComplexMeasurements(app: MathGraphApp, exercise: GeometryExercise) {
  ...
  for (const [key, expected] of Object.entries(exercise.validation_config.expectedMeasurements)) {
    ...
    if (measurement && Math.abs(measurement.getValue() - expected.value) <= expected.tolerance) {
      ...
      results.feedback.push(`❌ ${expected.description} : ${measurement ? 'valeur incorrecte' : 'mesure manquante'}`);
    }
  }
...
{
  "rubric": {
    "measurements_basic": { "points": 40, "description": "Mesures de base (côté, diagonale, rayon)" },
...
async function validatePerpendicularBisector(app: MathGraphApp, exercise: GeometryExercise): Promise<ValidationResults> {
...
```json
{
  "rubric": {
    ...
    "tangency_verified": { "points": 10, "description": "Le cercle est bien tangent aux trois côtés" }
  },
  "passing_score": 70
}
```
...
```typescript
async function validateGeometricLocus(app: MathGraphApp, exercise: GeometryExercise): Promise<ValidationResults> {
  ...
  if (M1 && isPointOnLine(M1, bisector1)) {
    ...
    if (Math.abs(dist1 - dist2) <= 3) {
      results.score += 17.5;
      results.feedback.push(`✅ Point M1 est équidistant de d1 (${dist1.toFixed(1)}mm) et d2 (${dist2.toFixed(1)}mm)`);
    } else {
      results.feedback.push(`❌ Point M1 n'est pas équidistant : d(M1,d1) = ${dist1.toFixed(1)}mm, d(M1,d2) = ${dist2.toFixed(1)}mm`);
    }
  }
  ...
  if (M2 && isPointOnLine(M2, bisector2)) {
    ...
    if (Math.abs(dist1 - dist2) <= 3) {
      results.score += 17.5;
      results.feedback.push(`✅ Point M2 est équidistant de d1 (${dist1.toFixed(1)}mm) et d2 (${dist2.toFixed(1)}mm)`);
    } else {
...
```json
{
  "rubric": {
    ...
    "second_bisector": { "points": 25, "description": "Deuxième bissectrice correcte et perpendiculaire" },
...
<div class="proof-container">
        ...
        <div class="step-justification">
          ...
          {#if step.justificationId}
            <div class="justification-description">
              {exercise.validation_config.availableJustifications.find(j => j.id === step.justificationId)?.description}
            </div>
          {/if}
        </div>
      </div>
  ...
  <button class="validate-button" onclick={validateProof}>
    Valider la démonstration
  </button>
</div>
```
...
```typescript
async function validateProof(studentProof: ProofStep[], exercise: GeometryExercise): Promise<ValidationResults> {
  ...
  for (const required of requiredSteps) {
    const studentStep = studentProof.find(step =>
      normalizeStatement(step.statement) === normalizeStatement(required.statement)
    );
...
```typescript
async function validateThalesProof(studentProof: ProofStep[], exercise: GeometryExercise): Promise<ValidationResults> {
  const results: ValidationResults = {
    passed: false,
    score: 0,
    maxScore: 100,
  ...
  const hasHypothesis = studentProof.some(step =>
  ...
  const hasAngleEquality = studentProof.some(step =>
  ...
  const hasSimilarity = studentProof.some(step =>
    step.statement.includes('semblables') &&
  ...
  const hasProportionalSides = studentProof.some(step =>
    step.statement.includes('proportionnel') &&
  ...
  const hasThreeRatios = studentProof.some(step =>
...
function isLogicallyOrdered(studentProof: ProofStep[], requiredSteps: RequiredStep[]): boolean {
  ...
  const conclusionIndex = studentProof.findIndex(step =>
    step.justificationId === 'conclusion'
  );
...
```json
{
  "rubric": {
    ...
    "proportional_property": { "points": 20, "description": "Propriété des côtés proportionnels utilisée" },
