...
2. **Différentiation symbolique** : `derivative` placeholder = `ZERO_NODE`, `compiledDerivative` = `() => 0`. Le sampler avec dérivée constante 0 dégrade vers un sampling quasi-uniforme. Acceptable visuellement pour V1 ; améliorable via différentiation par branche en post-D.
3. **Tangente sur piecewise** : `tangente(f, x0)` produirait une tangente horizontale (dérivée placeholder = 0). Non testé. À ajouter à la liste de "ne pas mélanger piecewise et tangente() en V1".
