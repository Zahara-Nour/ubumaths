# Prompt de continuation — Phase 7 : Tests end-to-end pipeline fill-in-blanks
...
**Phase 7 concerne la verification end-to-end** : s'assurer que le pipeline complet (transformer → generateur → composant) fonctionne sur les 633 questions migrees.
...
### 7.1 — Test transformer → generateur (integration)
...
### 7.2 — Echantillon representatif (au minimum)
...
### 7.3 — Test validation round-trip
...
### 7.4 — Test composant (AST rendering)
...
1. **Phase 7.0** : Proposer les comportements en francais, attendre validation
2. **Phase 7.1** : Ecrire les tests
3. **Phase 7.2** : Corriger les bugs trouves (si les tests echouent)
4. **Phase 7.3** : Code review + commit + doc progression

## Issues connues (des phases precedentes)

Ces issues ne bloquent pas la Phase 7 mais peuvent causer des echecs sur certaines questions :
