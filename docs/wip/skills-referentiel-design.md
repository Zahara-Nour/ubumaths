...
| 17                          | Distinction remédier/renforcer                                                                        | 🆘 « À remédier » (capacité **non acquise** avec ≥ 2 échecs récents bloquant la règle — cf. décisions 63 + 64) vs 🔁 « À renforcer » (capacité **acquise** > 30 j sans pratique — cf. décision 62). **Famille A uniquement** (cf. décision 39 : pas de badges famille B).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| ~~Q3~~  | ~~Référentiel partagé global ou par prof~~                          | **Tranché 2026-06-09** : **partagé global** en V1. Un seul référentiel BO 2026 commun à tous les profs (David rédige). Lecture publique des seeds référentiel ; pas de scope par prof. Impact RLS : `skill_themes` / `skill_objectives` / `skills` / `math_competences` / `math_competence_subdimensions` lisibles par tous les utilisateurs authentifiés. | ~~résolu~~                                |
| ~~Q4~~  | ~~Niveaux scolaires V1~~                                            | **Tranché 2026-06-09** : **6ᵉ seule** pour V1. Le référentiel 6ᵉ est rédigé (`referentiel/6e-savoirs.md`, 18 items × 4 capacités). 5ᵉ / 4ᵉ / 3ᵉ / 2nde / lycée reportés à des phases ultérieures (chaque nouveau niveau = ajout de lignes dans `skill_themes` / `skill_objectives` / `skills`, pas de changement de schéma).                               | ~~résolu~~                                |
...
| ~~Q10~~ | ~~Trame vide BO (cycle 4 + 2nde) à pré-générer~~                    | **Résolu 2026-06-08** : 6ᵉ produite et validée (`referentiel/6e-savoirs.md`, 18 items × 4 capacités). Cycle 4 + 2nde reportés (cf. Q4 toujours ouverte).                                                                                                                                                                                                   | ~~résolu~~                                |
...
### Côté David (utilisateur) — questions encore ouvertes

1. **Trancher Q7** : versionnement intra-année du référentiel (oui/non). Reportable si on assume « V1 fige le ref par année scolaire ».
2. **Trancher Q8 / Q9** : UX prof (saisie famille B) et UX élève (auto-éval). Reportables phases 5-6.

(Q1, Q2, Q3, Q4, Q5, Q6, Q10, Q11, Q12 résolues — cf. §11. Seul Q7 reste sur le périmètre du référentiel ; Q8/Q9 sont des questions d'UX qui pourront être tranchées au moment de coder l'UI.)

### Côté implémentation (phase 1 — migration DB)

1. **Schéma SQL Supabase** (une migration complète) — tables `skill_themes`, `skill_objectives`, `skills` (avec `family` generated column + `knowledge_type` + CHECK XOR), `math_competences`, `math_competence_subdimensions`, `evaluation_tasks`, `evaluation_task_perimeter`, `skill_attempts` (double régime), `question_template_skills`, caches `student_skill_state_a` / `student_observable_state` / `student_competence_level`.
2. **Fonctions PL/pgSQL** : triggers sur `INSERT skill_attempts` → recalcul des caches appropriés ; fonction `compute_competence_level(student_id, math_competence_id)` pour la règle conjonctive famille B (forme à trancher : PL/pgSQL vs TypeScript serveur).
3. **RLS policies** : élève ↔ ses propres données ; prof ↔ ses élèves via `class_members`. Lecture publique des seeds référentiel.
4. **Seeds** : 72 capacités depuis `referentiel/6e-savoirs.md` ; 56 observables + 22 sous-dimensions + 6 compétences math depuis `referentiel/college-competences.md`.
5. **Types TypeScript** + helpers (`src/lib/types/skills.ts`, `src/lib/types/database-helpers.ts`).
6. **Tagging des templates existants** (10 seeds dans `question_templates`) : créer les entrées `question_template_skills` correspondantes. Convention : un `template_id` distinct par variation canonique (décision 60). Anciens champs `theme/domain/subdomain/level/grades` conservés en cohabitation (décision 61).
7. **Branchement validation auto** : hook sur chaque réponse à une question UbuMaths → `INSERT skill_attempts (skill_id, template_id, success, with_help, source='auto')` pour chaque `skill_id` tagué sur le template.
8. **UI prof V1** : éditeur de référentiel + saisie d'attempts famille A (manuels post-éval) ; tâches d'évaluation famille B (CRUD + déclaration du périmètre + saisie en séance).
9. **UI élève V1** : dashboard 2 sections (objectifs famille A + 6 compétences math famille B) + vue détail (tableau 4 colonnes pour famille A ; observables groupés par sous-dimension + verdict explicité pour famille B).
...
- **2026-06-09 (suite 25 — analyse de cohérence finale + cleanup §11/§12)** : revue complète de cohérence du doc après les 7 ajustements de la session (décisions 62 à 68). Une incohérence trouvée et corrigée : décision 17 (§10) parlait encore de « 🆘 avec succès historique » alors que décisions 63 + 64 ont étendu le badge et ajouté le seuil minimum 2 échecs. Cleanup §11 Q10 (statut « ✓ en cours » → « ✓ résolu », 6ᵉ produite). §12 « Côté David » réécrit (les Q1/Q2/Q5/Q6/Q10/Q11/Q12 sont résolues, restent Q3/Q4/Q7). §12 « Côté implémentation » réécrit pour la phase 1 (mention explicite de `family` generated column, `knowledge_type`, `question_template_skills`, convention 1 template = 1 variation canonique, double régime `skill_attempts`, seeds 6e-savoirs + college-competences).
