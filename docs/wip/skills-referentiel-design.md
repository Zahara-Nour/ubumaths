...
- **Famille A — Connaissances et savoir-faire** : contenus disciplinaires (Nombres, Géométrie, Fonctions, etc.). Évaluées par des tâches techniques ciblées. Régime : **4 capacités binaires ordonnées par difficulté sous chaque objectif**. Niveau d'un objectif = max capacité acquise.
...
| -------------------- | ------------------------ | -------------------------------------------------------- | ---------------------------------------------------------------- |
| A — Connaissances    | 4–6 thèmes               | **~20 objectifs** par niveau scolaire                    | **exactement 4 capacités** par objectif (ordonnées par rang)     |
...
Pour la 6ᵉ : cible **~20 objectifs × 4 capacités = ~80 capacités** à rédiger en famille A.
...
Chaque Question liée à une capacité famille A est taguée avec **un seul `skill_id`** dans la junction `question_skills` (pas de `target_level`). Le tagging désigne **la capacité que la question évalue**.

**Tagging objectif, pas subjectif** : le concepteur identifie la capacité (par son nom, sa rubrique et son rang sous l'objectif) que la question teste effectivement. Pas de calibration de niveau ressenti. C'est un acte de **classification**, pas d'**évaluation**.

**Exemple** sur l'objectif « Comparer et ranger des nombres décimaux » (4 capacités de la section 3) :

| Question                                                     | Capacité taguée                                        |
| ------------------------------------------------------------ | ------------------------------------------------------ |
| « Compare 5,3 et 2,9 »                                       | Rang 1 — parties entières différentes                  |
| « Compare 4,17 et 4,32 »                                     | Rang 2 — mêmes parties entières, longueurs égales      |
| « Compare 2,5 et 2,15 »                                      | Rang 3 — mêmes parties entières, longueurs différentes |
| « Range dans l'ordre croissant : 3,2 ; 3,15 ; 3,201 ; 3,07 » | Rang 4 — ranger une liste justifiée                    |

**Couverture du pool (cf. §3)** : pour chaque capacité, viser **≥ 2-3 templates de questions distincts** qui en testent des variations. C'est cette diversité qui permet à la règle d'acquisition `capacite_attendue` (« ≥ 2 templates distincts ») de jouer son rôle anti-répétition triviale.

Validation auto :

- **Succès** (avec ou sans aide) → `skill_attempts (skill_id, success=true, with_help=?)`
- **Échec** → `skill_attempts (skill_id, success=false)` — stocké, utilisé pour le filtre des dernières tentatives

Une Question peut viser **plusieurs capacités/observables simultanément** (junction M2M sur `question_skills`). Une question d'une tâche à prise d'initiative en famille A peut typiquement aussi mobiliser des observables de famille B (par exemple « Calculer » + « Raisonner »).
...
**Famille A** : les badges 🆘 À remédier / 🔁 À renforcer sont conservés mais redéfinis.

```
Capacité non acquise avec ≥ 1 succès historique mais échec(s) récent(s)
qui empêchent la règle de rubrique de passer
  → marquée "à remédier" (🆘) : l'élève a déjà touché du doigt mais bute

Capacité acquise mais "à revoir" (last_success_at > 30 jours)
  → marquée "à renforcer" (🔁) : acquis qui faiblit par non-usage

Capacité acquise avec succès récent et règle satisfaite
  → état stable (pas de badge)
```

Une capacité **jamais touchée** (aucun attempt) n'est ni acquise ni à remédier : elle est simplement « non commencée » et cachée du dashboard d'entrée.
...
question_skills           (question_id, skill_id)                              -- famille A : tagging skill_id seul (modèle B)
                                                                               -- famille B : observable visé par la question
assessment_objectives     (assessment_id, objective_id)                       -- M2M
exercise_objectives       (exercise_id, objective_id)                         -- M2M

-- Junction templates ↔ skills : nécessaire pour compter distinct_template_successes
-- (règle d'acquisition capacite_attendue famille A — cf. §3 et §6.1)
-- Note : si chaque question = un template_id, on peut dériver depuis question_skills + questions.template_id.
...
student_skill_state_a (
  ...
  to_review                     boolean not null default false, -- décroissance > 30 jours sans succès
  needs_remediation             boolean not null default false, -- 🆘 succès historique + échecs récents
  needs_reinforcement           boolean not null default false, -- 🔁 acquise mais à revoir
...
- `question_skills.target_level` (caduc — modèle B : tagging skill_id seul)
...
| 56                          | **Champ `rubrique` sur famille A + adoption BO 2026 cycle 3**                                        | Refonte du référentiel 6ᵉ à partir du **BO 2026 cycle 3** (et non BO 2020). Le BO 2026 distingue deux rubriques par sous-domaine : `'automatisme'` et `'capacite_attendue'`. Champ `skills.rubrique` ajouté (nullable, famille A uniquement). **Option L littéral** : sous-domaine BO = objectif UbuMaths ; sous-headers internes (ex. « Distances », « Cercles » dans Configurations planes) = objectifs séparés. Total 6ᵉ : **25 objectifs / 127 capacités** sur 6 thèmes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ~~5, 7, 13–15, 18, 19, 22~~ | ~~Modèle famille A « 4 paliers d'indicateurs par capacité »~~                                        | **Toutes caduques** — remplacées par le modèle B (décision 57). L'échelle 1-4 par capacité avec `level_indicators`, le seuil K=3, la fenêtre 60j, la robustesse récente sur niveau visé, le tagging `target_level` et les seuils d'agrégation 80 % sur objectif sont **abandonnés**.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 57                          | **Pivot vers modèle B pour la famille A (référentiel 2016 style)**                                   | Hiérarchie : Thème → Objectif → **exactement 4 capacités ordonnées par difficulté** (rang 1 → 4). Chaque capacité **binaire** (acquise ou pas), **distincte** (pas un palier d'une même compétence). Niveau objectif = max rang acquis. Niveau 3 = « attendu pour tous » (BO), Niveau 4 = expert. **Tagging questions** : `skill_id` seul (pas de `target_level`) — acte de classification, pas de calibration. **Validation par variations dans les templates (approche 3)** : pas de table d'observables séparée, la diversité du pool incarne les variations. **Règle d'acquisition modulée par rubrique** : `capacite_attendue` → ≥ 1 succès sur ≥ 2 templates distincts + aucun échec dans les 3 dernières ; `automatisme` → ≥ 5 succès + ≥ 3 dans les 5 dernières. **Décroissance** : > 30 jours sans succès → capacité acquise marquée « à revoir ». Schéma DB : drop `level_indicators`, `is_progressive`, `target_level` ; cache `student_skill_state_a` refondu (binaire). Cible 6ᵉ : ~20 objectifs × 4 capacités = ~80 capacités. |
