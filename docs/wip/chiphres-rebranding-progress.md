...
- **Sprint 1 — Lexique partout** : 🟡 **EN COURS** — branche `feat/sprint1-lexique`, **PR #65** (auto-merge armé). `src/lib/config/lore.ts` créé (source unique) + termes câblés : boutons/nav (Empocher, Cabinet des Phynances, Sceller, Tamiser…), **Galopin/Galopine, Bataillon, Capitaine, Corvée, Décervelage, Corvée Domestique**. Détail (termes, décisions, méthodo & pièges, reste à faire) : **[`sprint1-lexique-progress.md`](./sprint1-lexique-progress.md)** + spec [`sprint1-lexique-spec.md`](./sprint1-lexique-spec.md). **Encore à faire** : notification→Décret, learning (hint/badge/streak/vipCard), economy, `chiffres → chiphres`, `maths → Mathres`, sous-titre « les Chiphres de la Chandelle Verte », pages 404/500 ubuesques, scope `.ts`.
