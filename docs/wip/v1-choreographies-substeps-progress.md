...
6. **Nouveau : `perpendiculaire @euclide`** (2026-05-21) livrée avec **deux voies** :
   - **`rayon_libre`** (défaut) — viewport-safe, indépendante de la position de `A` sur (AB). 7 sub-steps : arc en `P` de rayon `r = max(1.5 × d(P, (AB)), 1.5)` qui coupe (AB) en `A*` et `B*` → fade-in de `A*` et `B*` → composition avec `sub-mediatrice(A*, B*)` via `ctx.sub` (4 sub-steps) → `line-fade-in` de la perpendiculaire. La sub-mediatrice utilise le perpendiculaire-principal comme `subPrincipalId` (la médiatrice de `[A*B*]` IS la perpendiculaire cherchée). Voie recommandée quand `A` peut être loin du pied de la perpendiculaire.
