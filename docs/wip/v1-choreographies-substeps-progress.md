...
8. **Nouveau : `parallele @equerre`** (2026-05-21) livrée — 6 sub-steps avec règle-glissière pédagogique : (1) pose équerre coin sur A, bord horizontal sur (AB) ; (2) pose règle contre le bord vertical de l'équerre (sert de glissière, `secondaryInstrument: 'setSquare'` garde l'équerre visible) ; (3) glissement de l'équerre le long de la règle jusqu'à `C_final = A + n_toP × |d_perp|` (`secondaryInstrument: 'ruler'` garde la règle visible) ; (4) crayon trace le long du bord horizontal (longueur = `EQUERRE_HORIZONTAL_MATH_UNITS` = 3 unités math) ; (5) swap équerre → règle alignée sur la parallèle ; (6) crayon prolonge le tracé. `C_final` exposé comme charnière. Pattern `secondaryInstrument` détourné pour maintenir un 2e instrument visible sans le déplacer.
