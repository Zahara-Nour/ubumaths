...
8. **Nouveau : `parallele @equerre`** (2026-05-21) livrée — 5 sub-steps sur le même modèle que `perpendiculaire @equerre` mais avec le bord horizontal pour tracer la parallèle : (1) pose équerre coin sur A, bord horizontal sur (AB) ; (2) glissement perpendiculaire à (AB) jusqu'à `C_final = A + n_toP × |d_perp|`, bord horizontal passant alors par P ; (3) crayon trace le long du bord horizontal (longueur = `EQUERRE_HORIZONTAL_MATH_UNITS` = 3 unités math, ≈ LARGEUR à PPU 40) ; (4) swap équerre → règle alignée sur la parallèle ; (5) crayon prolonge le tracé. `C_final` exposé comme charnière.
