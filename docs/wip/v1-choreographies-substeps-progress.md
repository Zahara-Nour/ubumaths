...
7. **Nouveau : `perpendiculaire @equerre`** (2026-05-21) livrée — 3 sub-steps en deux temps de pose + tracé : (1) `compass-measure` pose de l'équerre coin sur A, bord aligné avec (AB) ; (2) `compass-measure` glissement le long de (AB) jusqu'à F = projection de P sur (AB), bord vertical passant alors par P ; (3) `ruler-trace` crayon trace la perpendiculaire le long du bord vertical (instrument primaire = `setSquare`, secondaire = `pencil`). Pied F exposé comme charnière. Orientation : rotation = `angle(u_AB) + 180°` si P est du côté CW (l'équerre n'a pas de flip horizontal, on tourne de 180° pour aligner le bord vertical vers P).
