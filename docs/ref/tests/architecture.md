...
- **`render` est asynchrone** (`vitest-browser-svelte` 3, depuis 2026-09-25) : toujours `await render(…)`, et `await unmount()` / `await rerender(…)`. Un helper qui appelle `render` devient `async`, et ses appels sont attendus. ⚠️ Un `render` oublié sans `await` monte quand même le composant (le montage précède le premier `await`) : **aucun test ne rougira**. D'où la garde `pnpm check:await-render` (AST), lancée par la CI (job Lint) et par `lint:fast` dès qu'un test client a changé.
