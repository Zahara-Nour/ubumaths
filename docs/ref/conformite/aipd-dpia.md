...
| **Mesures existantes**  | **Consentement parental Art. 8 implémenté** (mode lecture seule sans accord) ; filtre de langage (`bad-words`) ; accès enseignant aux messages de ses élèves ; pas de publicité ni de profilage commercial ; minimisation (champ `gender` supprimé) ; **classements de jeux limités à l'école** (retrait du classement public inter-écoles ; identité affichée = **prénom + avatar** seulement). |
...
| ------------ | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 16 juin 2026 | 0.4     | **Classements de jeux (safeguarding mineurs)** : suppression du **classement public global** (exposition de mineurs inter-écoles, identités anonymisées mais globales — vue `minesweeper_leaderboard_public`) au profit de classements **école-scopés** (classe / niveau / école) via fonctions `SECURITY DEFINER` bornées par `my_school()`. Identité affichée réduite à **prénom + avatar** (ni nom, ni e-mail). Accès **authentifié** requis (routes passées de `(public)` à `(protected)`). Base : minimisation (art. 5.1.c) + intérêt supérieur de l'enfant.                                                                                                                           |
