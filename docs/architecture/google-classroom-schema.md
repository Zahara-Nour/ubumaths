...
- Encrypted at rest using `pgcrypto` extension
- Symmetric encryption with `app.encryption_key`
- Key stored in database settings (should be rotated regularly)
- Decryption restricted to authorized users via RLS + SECURITY DEFINER

**Best Practices**:
1. Set encryption key in Supabase Dashboard → Settings → Database
...
1. Teacher initiates OAuth (redirect to Google)
2. Google redirects back with authorization code
3. Exchange code for `access_token` and `refresh_token`
4. Encrypt and store tokens in `google_integrations`
5. Use `access_token` for API requests
6. Refresh token when expired (check `token_expiry`)
