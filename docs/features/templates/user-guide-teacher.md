# 👨‍🏫 Guide Utilisateur - Templates de Messages (Professeur)
...
3. [Utiliser les Templates Système](#utiliser-les-templates-système)
4. [Créer vos Propres Templates](#créer-vos-propres-templates)
5. [Gérer vos Templates](#gérer-vos-templates)
6. [Variables et Personnalisation](#variables-et-personnalisation)
...
Un **template de message** est un modèle pré-rempli qui vous fait gagner du temps lors de la rédaction de messages aux étudiants. Au lieu de réécrire le même type de message à chaque fois, vous utilisez un template qui se remplit automatiquement avec les bonnes informations.
...
J'ai bien reçu votre question concernant l'évaluation
"Devoir Maison #3" qui est à rendre pour le 15 novembre.
...
2. Le système remplit automatiquement :
   - Le nom de l'étudiant (Marie)
   - Le titre de l'évaluation (Devoir Maison #3)
   - La date d'échéance (15 novembre)
   - Votre nom (M. Martin)
...
- **Vous ne pouvez pas les modifier** (mais vous pouvez les dupliquer !)
- Badge bleu "Système"
...
2. Allez dans **Dashboard** → **Mes Templates de Messages**
3. URL : `https://votresite.com/dashboard/teacher/message-templates`

### Interface

```
┌──────────────────────────────────────────────────────┐
│ [🔍 Rechercher...] [Filtres] [⭐ Favoris] [+ Nouveau] │
├──────────────────────────────────────────────────────┤
│ 📘 Template Système 1            [⭐] [📋] [⋯]      │
│ 📘 Template Système 2            [☆] [📋] [⋯]      │
│ 📗 Mon Template 1                [⭐] [📋] [✏️] [🗑️] │
...
- ✏️ : Modifier (vos templates uniquement)
- 🗑️ : Supprimer (vos templates uniquement)

---

## 📘 Utiliser les Templates Système

Les templates système sont créés par les administrateurs et couvrent les besoins courants de tous les professeurs.

### Voir les Templates Système

1. Utilisez le filtre **Scope** → Sélectionnez **"Système"**
2. Tous les templates système s'affichent
...
4. Vous pouvez maintenant le modifier comme vous le souhaitez !
...
- ✅ Vous avez besoin d'un template qui n'existe pas

### Pré-requis

⚠️ **Vous devez avoir au moins une classe** pour créer un template.

Si le bouton "+ Nouveau template" est grisé, c'est que vous n'avez pas encore de classe assignée.

### Étape 1 : Ouvrir le Formulaire

1. Cliquez sur **"+ Nouveau template"** en haut à droite
...
Écrivez le sujet du message avec des variables si nécessaire.
...
```
Rappel : Exercices pour {{class_name}}
```

```
Félicitations {{student_first_name}} !
```
...
4. Cliquez sur la variable pour l'insérer
...
**💡 Note** : Les variables disponibles dépendent du **Type de déclencheur** choisi.

### Étape 6 : Utiliser les Filtres (Optionnel)
...
**💡 Aide** : Cliquez sur **"❓ Aide sur les filtres"** pour voir tous les filtres avec des exemples.

### Étape 7 : Utiliser les Conditions (Optionnel)

Affichez du contenu seulement si une information existe.

#### Syntaxe

```
{{#if variable}}
  Contenu affiché si la variable existe
{{else}}
  Contenu affiché si elle n'existe pas (optionnel)
{{/if}}
```

#### Exemple : Afficher la Date d'Échéance si elle Existe
...
#### Exemple : Message Personnalisé pour Première Question
...
Avant de sauvegarder, vérifiez le rendu de votre template.

1. Cliquez sur l'onglet **"Prévisualisation"**
2. Le système affiche le template avec des données d'exemple
...
- ✅ Les conditions s'affichent correctement

**💡 Astuce** : Revenez à l'onglet **Édition** pour modifier, puis repassez à **Prévisualisation** pour voir les changements.

### Étape 9 : Enregistrer

1. Cochez **"Template actif"** pour le rendre utilisable immédiatement
   - ⚠️ Si décoché, le template sera créé mais pas utilisable
2. Cliquez sur **"Créer"**
3. ✅ Votre template est créé !
...
**Exemple** : Tapez "rappel" pour voir tous les templates de rappel.
...
**💡 Utilité** : Accès ultra-rapide à vos templates préférés.
...
#### 📜 Historique des Versions (Vos templates uniquement)
...
**⚠️ Note** : L'historique n'est disponible que pour **vos templates**. Les templates système ne montrent pas d'historique.
...
**⚠️ Note** : Vous ne pouvez modifier que **vos propres templates**. Les templates système ne peuvent pas être modifiés.

#### 🗑️ Supprimer (Vos templates uniquement)

1. Cliquez sur l'icône **corbeille** (🗑️)
2. Confirmez la suppression
3. ⚠️ Le template est supprimé définitivement

**⚠️ Attention** : La suppression est irréversible. Pensez à dupliquer avant de supprimer si vous voulez garder une copie !

#### 📤 Expand/Collapse

Cliquez sur le **chevron** (⌄) pour voir ou cacher le corps complet du template dans la liste.

**💡 Pratique** : Voyez rapidement le contenu sans ouvrir le template.
...
1. ✅ Utilisez l'onglet **Prévisualisation**
2. ✅ Vérifiez que toutes les variables sont remplacées
3. ✅ Relisez pour fautes d'orthographe
4. ✅ Testez avec un vrai message (à vous-même d'abord)
...
#### Q : Puis-je créer un template pour plusieurs de mes classes ?
...
#### Q : Que se passe-t-il si je supprime un template que j'ai déjà utilisé ?

**R** : Les messages déjà envoyés avec ce template restent intacts. Seul le template disparaît, vous ne pourrez plus l'utiliser pour de nouveaux messages.

### Variables et Filtres

#### Q : Comment savoir quelles variables sont disponibles ?

**R** : Cliquez sur **"# Insérer variable/filtre"** dans le formulaire. L'onglet **Variables** liste toutes les variables disponibles pour le type de déclencheur choisi.
...
4. ✅ Donnée existe ? Si la donnée n'existe pas, la variable reste vide

**Solution** : Utilisez une condition `{{#if}}` pour les variables optionnelles.

#### Q : Puis-je combiner plusieurs filtres ?

**R** : Oui ! Chaînez-les avec `|` :

```
{{student_name | lowercase | capitalize}}
```

L'ordre compte : les filtres s'appliquent de gauche à droite.
...
**R** : Oui, via l'éditeur (bouton image). L'image sera uploadée et intégrée au template.
...
- ✅ `{{variable | filtre}}` (correct)
- ❌ `{{variable|filtre}}` (manque espace avant |)
- ✅ `{{#if var}}...{{/if}}` (correct)
- ❌ `{{#if var}}...{{/end}}` (mauvaise balise de fermeture)
...
- **Aide sur les filtres** : Cliquez sur "❓ Aide sur les filtres" dans l'interface
- **Guide Admin** : `GUIDE_UTILISATEUR_ADMIN_TEMPLATES.md` (pour des détails techniques)
- **Guide rapide** : `GUIDE_DEMARRAGE_RAPIDE_TEMPLATES.md`

### Support

- **Questions** : Contactez votre administrateur ou le support technique
- **Suggestions** : Vos idées d'amélioration sont les bienvenues !
- **Problèmes techniques** : Contactez le support IT
...
Je vous rappelle [INSÉRER VOTRE RAPPEL ICI].
...
Bravo pour votre excellent travail sur {{assessment_title}} !
...
J'ai vu que vous aviez des difficultés avec {{topic}}.
