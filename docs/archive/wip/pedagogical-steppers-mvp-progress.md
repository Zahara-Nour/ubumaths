...
- **Intégration aux corrections de questions** (`QuestionCorrection.generatedSteps` Mode B) → livré, voir `correction-integration-progress.md`. Inclut :
  - Schéma type `GeneratedSteps` discriminé (`kind: 'arithmetic' | 'linear-equation' | 'differentiate' | 'quadratic-equation' | 'linear-inequality' | 'quadratic-inequality' | 'rational-inequality' | 'integrate' | 'simplify' | 'arithmetic-from-blank' | 'limit' | 'domain'`) — **12 kinds**
  ...
  - Page debug `/dashboard/admin/debug/correction-mode-b` pour validation visuelle (21 fixtures : arithmetic + linear-equation + differentiate × 2 + quadratic-equation + linear-inequality × 2 + quadratic-inequality × 2 + rational-inequality × 2 + integrate × 2 + simplify × 2 + arithmetic-from-blank + limit × 2 + domain × 3 ; la `rationalInequalityMultiFracDemo` V2 est en snapshot test mais pas affichée)
...
- Renderers pédagogiques pour les autres domaines : matrix uniquement (intégration livrée 2026-05-06, simplification livrée 2026-05-06, **limits livré 2026-05-07** V1 + V1.1 — Option B pipeline parallèle, 8 stratégies au total : direct-substitution, known-limit, factorisation, rationalisation, infinity-analysis, lhopital (sup), one-sided, squeeze/gendarmes — voir `limits-renderer-progress.md`, **domain V1 MVP + V1.1.a livré 2026-05-07** — Option C instrumentation directe `compute.ts`, 13 rules : V1 = sqrt/ln/log/division/arcsin/arccos + intersection ; V1.1.a = tan-cot-sec-csc, arccosh/arctanh (sup), power, even_root, format aligned 3-lignes, V1.2 = preimage\_\*, composition explicite, union/complement/difference — voir `domain-renderer-progress.md`)
