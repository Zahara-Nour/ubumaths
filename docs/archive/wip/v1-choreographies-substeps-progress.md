...
6. **Nouveau : `perpendiculaire @euclide`** (2026-05-21) livrée avec **deux voies** :
   - **`rayon_libre`** (défaut) — viewport-safe, indépendante de la position de `A` sur (AB). 7 sub-steps : 2 petits arcs en `P` de rayon `r = max(1.5 × d(P, (AB)), 1.5)` (un de chaque côté de F) coupent (AB) en `A*` et `B*` → fade-in `A*, B*` → 2 petits arcs en `A*` puis `B*` (même ouverture `r`) se croisent en `Q` = symétrique de `P` par rapport à (AB) → fade-in `Q` → règle `P→Q`. Voie recommandée quand `A` peut être loin du pied. Pas de composition `ctx.sub` (chorégraphie self-contained).
   - **`arcs_egaux`** (variante compacte) — 7 sub-steps « arcs égaux d'Euclide » : `compass-measure |PA|` → petit arc en `P` (même ouverture) coupe `(AB)` en `B'` → petits arcs en `A` et `B'` (toujours `|PA|`) se croisent en `Q` = symétrique de `P` par rapport à `(AB)` → règle `P→Q`. Réutilise l'ouverture du compas partout, mais devient illisible si `A` est loin du pied. Cas dégénéré `P ∈ (AB)` : `Q = P` (arcs tangents en P).
7. **Nouveau : `perpendiculaire @equerre`** (2026-05-21) livrée — 5 sub-steps en deux temps d'équerre + swap vers règle : (1) `compass-measure` pose de l'équerre coin sur A, bord aligné avec (AB) ; (2) `compass-measure` glissement de l'équerre le long de (AB) jusqu'à F ; (3) `ruler-trace` crayon trace la portion DANS l'équerre (longueur = `EQUERRE_EDGE_MATH_UNITS` = 5 unités math, ≈ hauteur du bord vertical à PPU 40) ; (4) `compass-measure` swap équerre → règle alignée sur la perpendiculaire au pied F ; (5) `ruler-trace` crayon prolonge la perpendiculaire le long de la règle (côté opposé à P). La ligne infinie est révélée par `applyFinalVisibility` pour couvrir l'éventuelle extension avant non tracée. Pied F exposé comme charnière. Orientation : rotation = `angle(u_AB) + 180°` si P est du côté CW (l'équerre n'a pas de flip horizontal).
8. **Nouveau : `parallele @equerre`** (2026-05-21) livrée — 6 sub-steps avec règle-glissière pédagogique : (1) pose équerre coin près du projeté F de P sur (AB) (décalé d'une demi-LARGEUR), bord horizontal sur (AB) ; (2) pose règle contre le bord vertical de l'équerre (sert de glissière, décalée de `RULER_WIDTH_MATH_UNITS = 57/40` pour éviter le chevauchement, `secondaryInstrument: 'setSquare'` garde l'équerre visible) ; (3) glissement de l'équerre le long de la règle jusqu'à `C_final = equerreInitial + n_toP × |d_perp|` (`secondaryInstrument: 'ruler'` garde la règle visible) ; (4) crayon trace le long du bord horizontal (longueur = `EQUERRE_HORIZONTAL_MATH_UNITS` = 3 unités math, P centré dans le tracé) ; (5) swap équerre → règle alignée sur la parallèle avec overlap ; (6) crayon prolonge le tracé. `C_final` exposé comme charnière. En mode `@complet` : segment-glissière + deux marqueurs d'angle droit (`marque: 'carre'`) aux deux perpendicularités, montrant la propriété de double-perpendiculaire. Pattern `secondaryInstrument` détourné pour maintenir un 2e instrument visible sans le déplacer.
9. **Instruments math-unit-cohérents** (2026-05-21) — `SetSquare`, `Ruler` et `Protractor` exposent désormais un prop `pixelsPerUnit` et scalent toutes leurs dimensions (rayon externe, hauteurs de graduations, largeurs de bord, etc.) par ce facteur. Conséquence : leur empreinte sur la grille est constante à n'importe quel zoom du canevas (alignée avec la convention `length × pixelsPerUnit` déjà utilisée pour la règle). Les offsets pré-calculés dans les chorégraphies `@equerre` (en unités math) restent visuellement corrects à tout zoom. Constantes math intrinsèques : équerre `LARGEUR_MATH = 131/40 ≈ 3.275`, `HAUTEUR_MATH = 223/40 ≈ 5.575` ; règle `WIDTH_MATH = 57/40 ≈ 1.425` ; rapporteur `RAY_MATH = 156/40 = 3.9`.
10. **Mediatrice refactorée en 4 petits arcs** (2026-05-21) — Au lieu de 2 grands arcs de 120° (un par centre de compas), la médiatrice trace désormais **4 petits arcs de 30°** : 2 au compas en A (vers I1 puis vers I2) et 2 au compas en B (idem). Aligne le pattern visuel avec bissectrice et perpendiculaire (figure moins chargée). Chaque petit arc passe exactement par son intersection cible grâce au partage du `radiusScalar` et des centres `A, B` avec les cercles cachés alimentant `createIntersectionCC`. Total : **6 sub-steps** (vs 4 avant). Cascade automatique sur `cercle_circonscrit @euclide` qui passe de **13 à 17 sub-steps** (la composition `ctx.sub('mediatrice', ...)` reprend la nouvelle séquence sans modification du code parent).
