...
class FSRS {
  ...
  calculateNewStability(card, grade) {
    ...
    const retrievabilityFactor = 
      Math.exp((1 - retrievability) * this.w[10]) - 1;
    
    const stabilityIncrease = 
      1 + difficultyFactor * stabilityFactor * retrievabilityFactor;
  ...
  {#if currentCard}
    ...
    <div class="card">
      ...
      {#if showAnswer}
        <div class="answer">
          ...
          <div class="grade-buttons">
            <button onclick={() => handleGrade(1)} class="again">
              ❌ Échec
            </button>
            <button onclick={() => handleGrade(2)} class="hard">
              😓 Difficile
            </button>
            <button onclick={() => handleGrade(3)} class="good">
              👍 Bien
            </button>
            <button onclick={() => handleGrade(4)} class="easy">
              ✨ Facile
            </button>
          </div>
        </div>
      ...
      <div class="card-metadata">
        <p>
          Difficulté: {currentCard.difficulty?.toFixed(1)}/10 | 
          Stabilité: {currentCard.stability?.toFixed(1)} jours |
          R: {(fsrs.calculateRetrievability(currentCard) * 100).toFixed(0)}%
        </p>
      </div>
    </div>
  {:else}
...
<style>
  ...
  .easy { background: #3498db; color: white; }
  .show-answer { 
    background: #9b59b6; 
    color: white; 
    width: 100%;
...
export const EDUCATIONAL_CONFIG = {
  ...
  desiredRetention: 0.90, // 90% par défaut
...
export const RETENTION_PROFILES = {
  relaxed: 0.80,   // Décontracté
  balanced: 0.90,  // Équilibré (recommandé)
...
const fsrs = new FSRS(
  ...
  EDUCATIONAL_CONFIG.desiredRetention
);
