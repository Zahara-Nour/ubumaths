...
describe('GET /api/student/shared-materials', () => {
	...
	beforeEach(() => {
		...
		// Type as any for test simplicity - we only need id and role
		...
		// Mock locals with typed Supabase client

		// Type as any for test simplicity - we only need supabase
	...
	describe('Happy Path', () => {
		it('should return paginated materials for student', async () => {
			...
			// Mock count query
			mockSupabase.in = vi.fn().mockResolvedValueOnce({
				count: 1,
				error: null
			});
	...
	describe('Database Errors (500)', () => {
		it('should handle class fetch errors', async () => {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			await expect(GET(event as any)).rejects.toThrow('Failed to fetch student classes');
		});
		...
		it('should handle materials fetch errors', async () => {
			...
			// eslint-disable-next-line @typescript-eslint/no-explicit-any
			await expect(GET(event as any)).rejects.toThrow('Failed to fetch materials');
		});
