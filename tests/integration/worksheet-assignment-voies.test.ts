/**
 * Les deux voies de distribution d'une fiche (nécessite une base locale)
 * =====================================================================
 *
 * Une fiche atteint un élève par deux chemins :
 *
 *   par la CLASSE — `worksheet_assignment_classes`, la jonction, qui porte
 *            toutes les classes visées ;
 *   par le NOM   — `worksheet_assignment_students`, l'élève désigné
 *            nommément, sans condition de classe : c'est le cas de l'élève
 *            hors classe.
 *
 * Il y en avait une troisième, `worksheet_assignments.class_id`, une colonne
 * historique qui ne dupliquait que la PREMIÈRE classe de la jonction. Elle a
 * été supprimée le 2026-09-12 après qu'un test de ce fichier eut prouvé
 * qu'elle ne portait rien que la jonction n'ait déjà.
 *
 * CE QUE CES TESTS PROUVENT. La correction du 2026-09-08 a étendu
 * `student_has_worksheet_access` à tous les chemins, mais a laissé de côté
 * `student_has_exercise_access(uuid)`, qui garde la policy « Students can view
 * assigned exercises » sur `exercises`. Résultat : par la jonction comme par la
 * désignation nominale, l'élève
...
describe('les deux voies de distribution', () => {
	...
	/** Élève membre de la première classe visée. */
	let elevePremiere: SupabaseClient<Database>;
	/** Élève membre de la SECONDE classe visée — celle que la colonne oubliait. */
	let eleveSeconde: SupabaseClient<Database>;
	/** Élève d'aucune classe, atteint par son seul nom. */
	...
	beforeAll(async () => {
		...
		// exactement ce que produit l'API de distribution.
	...
	it('par la classe — l’élève de la première classe lit la fiche ET ses exercices', async () => {
	...
	it('par la classe — l’élève de la SECONDE classe lit la fiche', async () => {
	...
	it('par la classe — et il lit AUSSI ses exercices', async () => {
	...
	it('par le nom — l’élève nommément désigné lit la fiche', async () => {
		expect(await litLaFiche(eleveDesigne)).toBe(true);
	});

	it('par le nom — et il lit AUSSI ses exercices', async () => {
	...
	it('une affectation annulée ne donne rien, par aucun chemin', async () => {
	...
	it('une classe ARCHIVÉE ne donne plus accès, mais l’élève désigné garde le sien', async () => {
		// `classes.is_active` garde le chemin par la classe, et aucun test
		// ne la protégeait : une réécriture pourrait la perdre sans rougir.
		//
		// La désignation nominale n'a, elle, aucune condition de classe — c'est le point de
