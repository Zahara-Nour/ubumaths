...
import { estContenuVide, resolveHomeworkItems } from '../journal-homework';
import type { ClassSessionCalendar } from '../class-sessions';

const SEANCE = '2026-09-10';
...
describe('estContenuVide', () => {
	...
	it('ne prend pas du texte pour du vide', () => {
		expect(estContenuVide('<p>Exercices 12 à 15</p>')).toBe(false);
	});

	it('ne jette pas un travail qui n’est qu’une image', () => {
		expect(estContenuVide('<p><img src="data:image/png;base64,xxx" /></p>')).toBe(false);
	});
	...
	it('ne jette pas un travail qui n’est qu’une formule', () => {
		...
		expect(estContenuVide('<p><span data-math-inline latex="x^2"></span></p>')).toBe(false);
	});

	it('ne jette pas une citation de ressource', () => {
		expect(estContenuVide('<p>[[exercise:0f8b0b3e-0000-4000-8000-000000000001|Fiche 3]]</p>')).toBe(
			false
		);
	});
});
...
describe('resolveHomeworkItems — échéance absente', () => {
	it('résout une échéance vide au prochain cours', () => {
		...
		const { items, refusees } = resoudre([{ content: '<p>Exercice 12</p>', dueDate: null }]);
	...
	it('traite une chaîne vide comme une échéance absente', () => {
		const { items } = resoudre([{ content: '<p>Exercice 12</p>', dueDate: '' }]);
...
describe('resolveHomeworkItems — échéance fournie', () => {
	it('accepte un jour de cours', () => {
		const { items, refusees } = resoudre([{ content: '<p>DM</p>', dueDate: '2026-09-24' }]);
	...
	it('refuse un jour sans cours', () => {
		...
		const { items, refusees } = resoudre([{ content: '<p>DM</p>', dueDate: '2026-09-23' }]);

		expect(refusees).toEqual(['2026-09-23']);
		expect(items).toEqual([]);
	});

	it('refuse un jour de cours tombant en vacances', () => {
		const { refusees } = resoudre([{ content: '<p>DM</p>', dueDate: '2026-10-22' }]);

		expect(refusees).toEqual(['2026-10-22']);
	});

	it('refuse la date de la séance elle-même', () => {
		const { refusees } = resoudre([{ content: '<p>DM</p>', dueDate: SEANCE }]);

		expect(refusees).toEqual([SEANCE]);
	});
...
describe('resolveHomeworkItems — travaux vides', () => {
	...
	it('ne refuse pas l’échéance d’un travail vide', () => {
		...
		const { refusees, items } = resoudre([{ content: '<p></p>', dueDate: '2026-09-23' }]);
