...
export class GoogleClassroomClient {
	...
	/**
	 * Find a course work material by title in a specific topic
	 *
	 * @param courseId - The Google Classroom course ID
	 * @param topicId - The topic ID to search in
	 * @param title - The title to match
	 * @returns The matching material or undefined if not found
	 */
	async findCourseWorkMaterialByTitle(
		courseId: string,
		topicId: string,
		title: string
	): Promise<GoogleCourseWorkMaterial | undefined> {
		if (!courseId) {
			throw new Error('Course ID is required');
		}

		// List all published materials
		const { courseWorkMaterial } = await this.listCourseWorkMaterials(courseId, {
			pageSize: 100,
			courseWorkMaterialStates: ['PUBLISHED']
		});
		...
		// Find matching material by title and topicId
		return courseWorkMaterial.find((m) => m.topicId === topicId && m.title === title);
	}

	/**
	 * Add materials to an existing course work material
	 *
	 * @param courseId - The Google Classroom course ID
	 * @param materialId - The course work material ID to update
	 * @param newMaterials - The new materials to add
	 * @returns The updated course work material
	 */
	async addMaterialsToCourseWorkMaterial(
		courseId: string,
		materialId: string,
		newMaterials: GoogleMaterial[]
	): Promise<GoogleCourseWorkMaterial> {
		if (!courseId) {
			throw new Error('Course ID is required');
		}
		...
		// First, get the existing material to retrieve current materials
		const endpoint = `/courses/${courseId}/courseWorkMaterials/${materialId}`;
		const existingData = await this.request<GoogleCourseWorkMaterial>(endpoint);
		...
		// Patch the material with updated materials list
		const patchEndpoint = `/courses/${courseId}/courseWorkMaterials/${materialId}?updateMask=materials`;
		const data = await this.request<GoogleCourseWorkMaterial>(patchEndpoint, {
			method: 'PATCH',
