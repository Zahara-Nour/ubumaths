...
describe('Linear Solver', () => {
	...
	describe('Verbosity levels', () => {
		...
		it('should have French descriptions in steps', () => {
			...
			// Phase 6 — solve() now records only structural algorithmic steps
			// (identify-linear-coefficients + isolate-variable). Pedagogical
			// narrative is generated by `pedagogical-solve/linear` instead.
			// Verify FR descriptions appear in the algorithmic steps.
			const descriptions = result.steps.map((s) => s.description);
			expect(descriptions.some((d) => d.includes('identifie') || d.includes('Solution'))).toBe(
				true
			);
		});
	});
