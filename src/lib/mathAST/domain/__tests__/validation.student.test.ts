...
describe('compareDomains', () => {
	it('should detect equal domains', () => {
		const d1 = intervalDomain([greaterThanInterval(number(0))]);
		const d2 = intervalDomain([greaterThanInterval(number(0))]);
	...
	it('should detect mixed error (missing and extra)', () => {
		const student = intervalDomain([greaterThanInterval(number(1))]); // ]1, +∞[
		const correct = intervalDomain([openInterval(number(0), number(5))]); // ]0, 5[
...
describe('domainsAreEqual', () => {
	...
	it('should return false for different domains', () => {
		const d1 = intervalDomain([greaterThanInterval(number(0))]);
		const d2 = intervalDomain([greaterThanOrEqualInterval(number(0))]);
...
describe('calculateDomainSimilarity', () => {
	it('should return 100 for equal domains', () => {
		const domain = intervalDomain([greaterThanInterval(number(0))]);
	...
	it('should return partial credit for subset', () => {
		const student = intervalDomain([greaterThanInterval(number(0))]);
