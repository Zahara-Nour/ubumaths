...
class PrattParser {
	 ...
	 *
	 * These are paired delimiters that map to floor() and ceil() functions.
	 * Also handles \left\lfloor x \right\rfloor syntax.
	 */
	...
	private shouldInsertImplicitMultiply(): boolean {
		...
		// Check for closing floor/ceil delimiters
		if (token.type === 'COMMAND' && (token.value === 'rfloor' || token.value === 'rceil')) {
			return false;
		}
