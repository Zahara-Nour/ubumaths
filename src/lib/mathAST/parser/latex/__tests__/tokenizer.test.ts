...
describe('Number tokenization', () => {
	...
	describe('Scientific notation', () => {
		...
		it('should tokenize leading decimal with exponent', () => {
			const tokens = tokenize('.5e10');
			expect(tokens[0].value).toBe('.5e10');
		});
		...
		it('should correctly tokenize scientific notation followed by operator', () => {
			const tokens = filterWhitespace(tokenize('2e3+1'));
		...
		it('should tokenize leading zero decimal with exponent', () => {
			const tokens = tokenize('.0e5');
			expect(tokens[0].value).toBe('.0e5');
		});
		...
		it('should tokenize scientific notation followed by subtraction', () => {
			const tokens = filterWhitespace(tokenize('1e10-5'));
			expect(tokens[0].value).toBe('1e10');
		...
		it('should tokenize scientific notation followed by multiplication', () => {
			const tokens = filterWhitespace(tokenize('1e10*2'));
			expect(tokens[0].value).toBe('1e10');
		...
		it('should tokenize two consecutive scientific notation numbers', () => {
			const tokens = filterWhitespace(tokenize('1e10 2e5'));
			expect(tokens[0].value).toBe('1e10');
			expect(tokens[1].value).toBe('2e5');
		});

		it('should tokenize scientific notation in fraction context', () => {
			const tokens = filterWhitespace(tokenize('\\frac{1e10}{2e5}'));
			expect(tokens[2].value).toBe('1e10');
			expect(tokens[5].value).toBe('2e5');
		});

		it('should NOT consume e when followed by equals', () => {
			// '1e=' should be NUMBER("1") + LETTER("e") + EQUALS
			const tokens = tokenize('1e=');
			expect(tokens[0].value).toBe('1');
			expect(tokens[1].value).toBe('e');
		...
		it('should NOT consume e when followed by less-than', () => {
			// '1e<' should be NUMBER("1") + LETTER("e") + LESS
			const tokens = tokenize('1e<');
			expect(tokens[0].value).toBe('1');
			expect(tokens[1].value).toBe('e');
		...
		it('should NOT treat ee as scientific notation', () => {
			// '1ee2' should be NUMBER("1") + LETTER("e") + LETTER("e") + NUMBER("2")
	...
	// -------------------------------------------------------------------------
	// French decimal comma {,} (LaTeX notation: avoids extra space after ,)
	// Generated by lib/utils/french-math.ts:toFrenchDecimal as the canonical
	// French rendering of decimals. Without this support, the round-trip
	// toFrenchDecimal → parseLatex breaks 100 % of the time.
	// -------------------------------------------------------------------------

	describe('French decimal comma', () => {
		it('tokenizes 1{,}5 as NUMBER 1.5', () => {
		...
		it('tokenizes 123{,}456 as NUMBER 123.456', () => {
		...
		it('tokenizes 0{,}5 as NUMBER 0.5', () => {
			const tokens = tokenize('0{,}5');
			expect(tokens[0].value).toBe('0.5');
		});

		it('tokenizes 1{,}5 + 2{,}3 as two numbers separated by +', () => {
			const tokens = filterWhitespace(tokenize('1{,}5 + 2{,}3'));
			expect(tokens[0].value).toBe('1.5');
			expect(tokens[1].type).toBe('PLUS');
			expect(tokens[2].value).toBe('2.3');
		});

		it('supports French comma in scientific notation: 1{,}5e2', () => {
			const tokens = tokenize('1{,}5e2');
			expect(tokens[0].value).toBe('1.5e2');
		});

		// --- Edge cases that must NOT be misinterpreted as decimal separators ---

		it('does NOT consume {,} at the start of a token (no preceding digit)', () => {
			// {,}5 was already invalid before this change — it should remain
			// untouched by scanNumber (which is only entered when current char
			// is a digit or a leading '.').
			const tokens = tokenize('{,}5');
			// Whatever the tokenizer produces, the first token must NOT be NUMBER('.5')
			expect(tokens[0].type).not.toBe('NUMBER');
		});

		it('does NOT consume {,} after a regular decimal point', () => {
			// 1.5{,}3 has no semantic meaning — the {,} after the '.' must not
			// be absorbed (hasDecimal guard). The number stops at 1.5.
			const tokens = tokenize('1.5{,}3');
			expect(tokens[0]).toEqual({
				type: 'NUMBER',
				value: '1.5',
				position: 0,
			...
			// Subsequent tokens are { , } 3 — left to the parser to reject.
		});

		it('does NOT confuse with empty group {} or other commas', () => {
			// {} is a different token sequence; no leading digit, no {,} pattern.
			const tokens = tokenize('1{}5');
			expect(tokens[0].value).toBe('1');
			expect(tokens[1].type).toBe('LBRACE');
		});

		// --- Pathological cases (toFrenchDecimal doesn't produce these,
		//     but the tokenizer must remain consistent.) ---

		it('handles dangling 1{,} at end of buffer (no fractional part)', () => {
			// Edge: input ends right after {,}. The current behaviour absorbs
			// {,} as the decimal separator; value becomes '1.' (trailing dot)
			// — accepted by the tokenizer as a valid decimal opener (mirrors
			// '42.' which is also tokenisable as NUMBER).
		...
		it('rejects double French comma 1{,}{,}5 (single decimal separator)', () => {
			// hasDecimal becomes true after the first {,}; the second {,}
			// is no longer matched as a decimal separator and falls out of
			// scanNumber. Result: NUMBER '1.' followed by {, , } 5 tokens.
			const tokens = tokenize('1{,}{,}5');
			expect(tokens[0]).toEqual({
				type: 'NUMBER',
				value: '1.',
