 ...
 *
 * Features:
 * - WILDCARD tokens: `_x` or `_x:constraint` become P._('x') or P._('x', P.isNumber())
 * - Standard math operators: +, -, *, /, ^
 * - Functions: sin(_x), cos(_x), etc.
 * - Parentheses: (_x + _y)
 * - Literals: numbers and variables
 *
 * @example
 * parsePattern('_x + _y')        // => P.add(P._('x'), P._('y'))
 * parsePattern('_n:number')      // => P._('n', P.isNumber())
 * parsePattern('sin(_x)')        // => P.func('sin', [P._('x')])
 *
...
export class PatternPrattParser {
	...
	/**
	 * Parse a wildcard pattern: _x or _x:constraint
	 */
	 ...
	 */
	private parseVariable(): Pattern {
		const token = this.advance();
		return P.var(token.value);
	}
 ...
 *
 * @example
 * parsePattern('_x')                 // => P._('x')
 * parsePattern('_n:number')          // => P._('n', P.isNumber())
 * parsePattern('_x + _y')            // => P.add(P._('x'), P._('y'))
 * parsePattern('_x + 0')             // => P.add(P._('x'), P.num(0))
 * parsePattern('_a * _b + _c')       // => P.add(P.mul(P._('a'), P._('b')), P._('c'))
 * parsePattern('-_x')                // => P.neg(P._('x'))
 * parsePattern('_x^2')               // => P.pow(P._('x'), P.num(2))
 * parsePattern('sin(_x)')            // => P.func('sin', [P._('x')])
 * parsePattern('(_x + _y)')          // => P.paren(P.add(P._('x'), P._('y')))
 */
