...
/**
 * Checks if a date is within a period's date range
 *
 * USAGE:
 * ------
 * Use this to determine if a specific date falls within a period.
 * Useful for filtering historical data by period.
 *
 * @param date - Date to check (can be Date object or ISO string)
 * @param period - Academic period object
 * @returns True if date is within period's range (inclusive)
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function isDateInPeriod(date: Date | string, period: AcademicPeriod): boolean {
 ...
 * USAGE:
 * ------
 * Filter periods by school year ID. Useful when displaying data
 * for a specific academic year.
 *
 * @param periods - All academic periods
 * @param schoolYearId - School year ID to filter by
 * @returns Periods belonging to the specified school year
 *
 * @example
 * ```typescript
 * const periods = [
 *   { id: 'p1', school_year_id: 'year-2024', ... },
 *   { id: 'p2', school_year_id: 'year-2024', ... },
 *   { id: 'p3', school_year_id: 'year-2023', ... },
 * ];
 * getPeriodsForYear(periods, 'year-2024'); // → [p1, p2]
 * ```
 */
