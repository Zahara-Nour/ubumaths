 ...
 *
 * Construction au compas en 4 gestes, en réutilisant l'ouverture `|PA|`
 * partout :
 *
 ...
 *
 * Justification mathématique : `|QA| = |QB'| = |PA|` (puisque
 * Q = 2F − P et que F est milieu de [AB']), donc Q est sur le cercle
 * de centre A de rayon `|PA|` ET sur le cercle de centre B' de rayon
 * `|PA|`. Et `|PA| = |PB'|` (par symétrie autour de F), donc B' est
 * sur le cercle de centre P de rayon `|PA|`.
 *
 * Cas dégénéré P ∈ (AB) (`d = 0`) : Q = P et les arcs sont tangents en
 * P. La construction reste cohérente (le compas-mesure et l'arc à P
 * affichent quand même B'), mais Q n'apparaît pas distinct de P. À
 * éviter en usage pédagogique courant.
 *
 * Sub-step layout :
 *   SS1 — compass-measure |PA| (compas en P, ouverture vers A).
 *   SS2 — compass-draw petit arc en P, sweep autour de la direction P→B'.
 *   SS3 — point-fade-in B'.
 *   SS4 — compass-draw petit arc en A, sweep autour de la direction A→Q.
 *   SS5 — compass-draw petit arc en B', sweep autour de la direction B'→Q.
 *   SS6 — point-fade-in Q.
 *   SS7 — ruler-trace de la perpendiculaire, line-fade-in m.
 *
 * Total : 7 sub-steps.
 */
...
export const VOIES_PERPENDICULAIRE_EUCLIDE: readonly Voie[] = [
	{
		...
		description:
			"Le compas en `P` ouvert à `|PA|` trace un petit arc qui coupe `(AB)` en `B'`. La même ouverture reportée en `A` puis en `B'` produit deux arcs qui se croisent en `Q`, symétrique de `P` par rapport à `(AB)`. La droite `(PQ)` est la perpendiculaire à `(AB)` passant par `P`.",
		defaut: true,
