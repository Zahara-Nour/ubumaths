<!--
	Manage Shared Material Dialog
	===============================

	Dialog for managing already-shared Google Classroom materials

	FEATURES:
	- View all classes material is shared with
	...
	PROPS:
	- material: The material being managed with sharing info
...
<script lang="ts">
	...
	interface SharedMaterialRecord {
		id: string;
		classId: string;
		className: string;
		visible: boolean;
	...
	let sharedRecords = $state<Map<string, SharedMaterialRecord>>(new Map());
	...
	/**
	 * Fetch full shared_materials records to get IDs and current values
	 */
	async function fetchSharedMaterialRecords() {
		loading = true;
		try {
			// Fetch shared material records for this material
			const response = await fetch(
				`/api/google/shared-materials?page=1&limit=100&materialId=${material.id}`
			);

			if (!response.ok) {
				throw new Error('Failed to fetch shared materials');
			}

			const data = await response.json();
			// Server now filters by materialId, so all records are relevant
			const relevantRecords = data.sharedMaterials || [];

			// Initialize records map
			const recordsMap = new Map<string, SharedMaterialRecord>();
	...
	async function handleSave() {
		...
		try {
			...
			const promises = modifiedRecords.map(async (record) => {
				const response = await fetch(`/api/google/shared-materials/${record.id}`, {
					method: 'PATCH',
					headers: { 'Content-Type': 'application/json' },
					body: JSON.stringify({
						visible: record.visible,
						categoryId: record.categoryId || null,
						descriptionOverride: record.customDescription || null
					})
				});
	...
	async function handleRemove(classId: string) {
		...
		try {
			// Use new endpoint with shared_materials record ID
			const response = await fetch(`/api/google/shared-materials/${record.id}`, {
				method: 'DELETE'
			});
...
<Dialog.Root open={true} onOpenChange={onClose}>
	<Dialog.Content class="max-h-[90vh] max-w-3xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title>Gérer : {material.title}</Dialog.Title>
			<Dialog.Description
		...
		<div class="space-y-4 py-4">
			...
			{:else if recordsList.length === 0}
				<div class="py-8 text-center text-muted-foreground">
					<p>Ce matériel n'est partagé avec aucun {lore.entities.class}</p>
				</div>
			{:else}
				<div class="space-y-3">
