<!--
	BonusHistoryModal Component
	===========================
	Modal dialog displaying the history of bonus changes for a student.

	Shows when bonuses were:
	- Earned (green, + icon)
	- Used (orange, arrow icon)
	- Removed (red, X icon)
...
<script lang="ts">
	...
	// Type for bonus history entries (matches API response)
	...
	let history = $state<BonusHistoryEntry[]>([]);
	let loading = $state(true);
	let errorMessage = $state<string | null>(null);

	// Get current bonus count from cache
	const currentBonus = $derived(studentCache.getRewardsSync()?.bonus ?? 0);
	...
	async function fetchHistory() {
		...
		try {
			const response = await fetch('/api/student/bonus-history');
	 ...
	 * - Used: delta < 0 (but not removed)
	 * - Earned: delta > 0
	 */
	function getEntryType(entry: BonusHistoryEntry): 'earned' | 'used' | 'removed' {
...
>
	...
	<div class="mb-6 flex items-center justify-between">
		<div>
			<h2 id="bonus-history-title" class="flex items-center gap-2 text-2xl font-bold">
				<Star class="h-6 w-6 fill-amber-400 text-amber-400" />
				Journal des Bonus
			</h2>
			<p class="mt-1 text-sm text-muted-foreground">
				Solde actuel : <span class="font-semibold text-amber-600">{currentBonus}</span> bonus
			</p>
		</div>
		<button
			onclick={handleClose}
