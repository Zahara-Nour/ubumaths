...
<T.Mesh rotation.y={rotation} position={[dicePosition.x, dicePosition.y, dicePosition.z]}>
...
<script>
	...
	let speed = $derived.by(() => {
		return Math.sqrt(diceVelocity.x ** 2 + diceVelocity.y ** 2 + diceVelocity.z ** 2);
	});
</script>
...
<script>
	...
	function createD10Geometry(r) {
		...
		const faces = [
			0, 2, 3, 0, 3, 4, 0, 4, 5, 0, 5, 6, 0, 6, 7, 0, 7, 8, 0, 8, 9, 0, 9, 10, 0, 10, 11, 0, 11, 2,
			1, 3, 2, 1, 4, 3, 1, 5, 4, 1, 6, 5, 1, 7, 6, 1, 8, 7, 1, 9, 8, 1, 10, 9, 1, 11, 10, 1, 2, 11
		];
...
<RigidBody
	...
	angularDamping={0.1}
>
	<AutoColliders shape="convexHull" density={1.0} restitution={0.4} friction={0.6}>
		<T.Mesh>
			<T.IcosahedronGeometry args={[1, 0]} />
...
<!-- Sol statique -->
<Collider shape="cuboid" args={[50, 0.1, 50]} friction={0.7} restitution={0.3} />
...
<script>
	...
	onMount(() => {
		...
		for (let faceIndex = 0; faceIndex < 6; faceIndex++) {
			...
			const uvs = [
				u,
				v + tileSize, // Bottom-left
				u + tileSize,
				v + tileSize, // Bottom-right
				u,
				v, // Top-left
				u + tileSize,
				v // Top-right
...
{#each trailPositions as pos, i}
	<T.Mesh position={[pos.x, pos.y, pos.z]}>
		...
		<T.MeshBasicMaterial color="#ff0000" opacity={i / trailPositions.length} transparent />
	</T.Mesh>
{/each}
```
...
export function createDiceStore() {
	...
	const totalValue = $derived(currentResults.reduce((sum, r) => sum + (r.value || 0), 0));
...
<script>
	...
	let { id, type = 'd20', color = '#ffffff', initialPosition = { x: 0, y: 5, z: 0 } } = $props();
...
>
	<AutoColliders shape="convexHull" restitution={0.4} friction={0.6}>
		<T.Mesh bind:ref={mesh} castShadow>
			<T is={geom.component} args={geom.args} />
			<T.MeshStandardMaterial
...
<div class="container">
	<div class="ui">
		...
		<div class="controls">
			<button onclick={() => diceStore.addDie('d20')}> Ajouter D20 </button>
			<button onclick={() => diceStore.addDie('d6')}> Ajouter D6 </button>
			<button
