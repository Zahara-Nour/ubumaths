...
async function renderAxes(viewport: Viewport) {
	const { container } = await render(AxisLines, {
		viewport,
 ...
 */
describe('AxisLines', () => {
	it('n’écrit jamais deux étiquettes à moins de 50 px', async () => {
		for (const viewport of [
			...
			const { xs, ys } = await renderAxes(viewport);
	...
	it('gradue les deux axes même quand leurs échelles diffèrent', async () => {
		const { labels } = await renderAxes({ xMin: -10, xMax: 10, yMin: -1000, yMax: 1000 });

		expect(labels.length).toBeGreaterThanOrEqual(8);
	});

	it('n’écrit aucune graduation sur une fenêtre dégénérée', async () => {
		const { labels } = await renderAxes({
			xMin: 1e6 - 5e-10,
			xMax: 1e6 + 5e-10,
			yMin: 1e6 - 5e-10,
			yMax: 1e6 + 5e-10
