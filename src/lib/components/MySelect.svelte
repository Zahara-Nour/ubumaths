...
{#if type === 'multiple'}
	<Select.Root type="multiple" bind:value={value as string[]} onValueChange={handleMultipleValueChange} {...restProps}>
...
{:else}
	<Select.Root type="single" bind:value={value as string} onValueChange={handleSingleValueChange} {...restProps}>
	<Select.Trigger class={computedTriggerClass} aria-label={placeholder}>
		{selectedLabel}
	</Select.Trigger>
	<Select.Portal>
