...
describe('resolveText', () => {
	...
	describe('Questions syntax', () => {
		...
		it('should resolve multiple variables', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('5 + 10 = 15');
		});

		it('should preserve surrounding text', () => {
			const text = 'The answer is {@:value} units.';
			const resolved: ResolvedVariable[] = [{ name: 'value', value: '42' }];
			expect(resolveText(text, resolved, 'questions')).toBe('The answer is 42 units.');
		});

		it('should handle variable at start', () => {
			const text = '{@:value} is the answer';
			const resolved: ResolvedVariable[] = [{ name: 'value', value: '42' }];
			expect(resolveText(text, resolved, 'questions')).toBe('42 is the answer');
		});

		it('should handle variable at end', () => {
			const text = 'The answer is {@:value}';
			const resolved: ResolvedVariable[] = [{ name: 'value', value: '42' }];
			expect(resolveText(text, resolved, 'questions')).toBe('The answer is 42');
		});
		...
		it('should handle consecutive variables', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('123');
		});
		...
		it('should handle same variable multiple times', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('5 + 5 = 10');
		});

		it('should throw error on undefined variable', () => {
			const text = 'Value: {@:undefined}';
			const resolved: ResolvedVariable[] = [{ name: 'a', value: '5' }];
			expect(() => resolveText(text, resolved, 'questions')).toThrow(
	...
	describe('Markdown syntax', () => {
		it('should resolve single variable', () => {
			const text = 'Value is {{a}}';
			const resolved: ResolvedVariable[] = [{ name: 'a', value: '42' }];
			expect(resolveText(text, resolved, 'markdown')).toBe('Value is 42');
		});
		...
		it('should resolve multiple variables', () => {
			...
			expect(resolveText(text, resolved, 'markdown')).toBe('5 + 10 = 15');
		});

		it('should preserve surrounding text', () => {
			const text = 'The answer is {{value}} units.';
			const resolved: ResolvedVariable[] = [{ name: 'value', value: '42' }];
			expect(resolveText(text, resolved, 'markdown')).toBe('The answer is 42 units.');
		});
		...
		it('should handle consecutive variables', () => {
			...
			expect(resolveText(text, resolved, 'markdown')).toBe('123');
		});

		it('should throw error on undefined variable', () => {
			const text = 'Value: {{undefined}}';
			const resolved: ResolvedVariable[] = [{ name: 'a', value: '5' }];
			expect(() => resolveText(text, resolved, 'markdown')).toThrow(
				'Variable "undefined" not found in resolved variables'
			);
		});
	});
	...
	describe('Both syntaxes', () => {
		it('should handle both syntaxes in same text', () => {
			const text = 'Questions: {@:a}, Markdown: {{b}}';
			const resolved: ResolvedVariable[] = [
				{ name: 'a', value: '5' },
				{ name: 'b', value: '10' }
			];
			expect(resolveText(text, resolved, 'both')).toBe('Questions: 5, Markdown: 10');
		});

		it('should handle interleaved syntaxes', () => {
			...
			expect(resolveText(text, resolved, 'both')).toBe('1 and 2 and 3');
		});
		...
		it('should default to both syntax', () => {
			...
			// No syntax parameter - should default to 'both'
			expect(resolveText(text, resolved)).toBe('1 and 2');
		});
	});
	...
	describe('LaTeX context', () => {
		it('should handle variables in LaTeX expressions', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('$$\\frac{3}{4}$$');
		});
		...
		it('should handle variables in inline LaTeX', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('Calculate $\\sqrt{16}$');
		});
		...
		it('should handle complex LaTeX formula', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('$$\\sum_{i=1}^{10} i = 55$$');
		});
		...
		it('should preserve LaTeX escapes', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('$$\\frac{10}{2} \\times \\sqrt{9}$$');
		});
	});
	...
	describe('Special characters and formatting', () => {
		it('should preserve whitespace', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('  5  +  10  ');
		});
		...
		it('should preserve newlines', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('Line 1: 5\nLine 2: 10');
		});
		...
		it('should handle special characters around variables', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('$99 (€85)');
		});
		...
		it('should handle punctuation', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('1, 2; 3!');
		});
		...
		it('should handle Unicode characters', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('∑ 10 → 55');
		});

		it('should handle quotes', () => {
			const text = 'She said "{@:value}" loudly';
			const resolved: ResolvedVariable[] = [{ name: 'value', value: 'hello' }];
			expect(resolveText(text, resolved, 'questions')).toBe('She said "hello" loudly');
		});
	...
	describe('Edge cases', () => {
		...
		it('should handle very long variable values', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe(`Long: ${longValue}`);
		});
		...
		it('should handle variable names with underscores', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('Value: 42');
		});
		...
		it('should handle text with only variables', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('content');
		});
		...
		it('should handle variable values that look like tokens', () => {
			...
			expect(resolveText(text, resolved, 'questions')).toBe('Value: {@:something}');
		});

		it('should handle HTML-like content', () => {
			const text = '<div>Value: {@:a}</div>';
			const resolved: ResolvedVariable[] = [{ name: 'a', value: '42' }];
			expect(resolveText(text, resolved, 'questions')).toBe('<div>Value: 42</div>');
		});
	});
});
