...
describe('detectCircularDependencies', () => {
	...
	describe('Cycles in complex expressions', () => {
		it('should not detect cycle when variable ref is inside random spec bounds', () => {
			const variables: Variable[] = [
				{ name: 'a', expression: '{#:1-{@:b}}' },
				{ name: 'b', expression: '{@:a}' }
			];
			const result = detectCircularDependencies(variables);
			// The implementation now correctly parses inside random spec bounds
			// and detects the circular dependency: a depends on b (in bounds), b depends on a
			expect(result.valid).toBe(false); // Correctly detected as circular
			expect(result.errors).toHaveLength(1);
			expect(result.errors[0].path).toContain('a');
			expect(result.errors[0].path).toContain('b');
			expect(result.errors[0].path).toContain('b');
		...
		it('should detect cycle when variable names used in eval', () => {
			const variables: Variable[] = [
	...
	describe('Markdown syntax cycles', () => {
		...
		it('should detect cycle with mixed syntax', () => {
			const variables: Variable[] = [
				{ name: 'a', expression: '{@:b}' },
				{ name: 'b', expression: '{{c}}' },
				{ name: 'c', expression: '{@:a}' }
