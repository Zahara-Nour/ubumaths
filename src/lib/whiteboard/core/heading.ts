...
/**
 * Get the optimal heading for a binding point, considering the target direction.
 * This chooses the heading that best aligns with the direction toward the target,
 * while still respecting which side of the shape the point is on.
 *
 * @param shape - The shape element
 * @param bindingPoint - The binding point on the shape
 * @param targetPoint - The target point (other end of the arrow)
 * @returns The optimal heading direction
 */
export function getOptimalHeadingForBinding(
	shape: ShapeElement,
	bindingPoint: Point,
	targetPoint: Point
): Heading {
	...
	const dx = targetPoint.x - bindingPoint.x;
	const dy = targetPoint.y - bindingPoint.y;

	// Determine which side of the shape the binding point is on
	const onLeft = bindingPoint.x <= bounds[0] + 1;
	const onRight = bindingPoint.x >= bounds[2] - 1;
	const onTop = bindingPoint.y <= bounds[1] + 1;
	const onBottom = bindingPoint.y >= bounds[3] - 1;

	// If clearly on a side, use that heading if it makes sense toward target
	if (onLeft && dx < 0) return HEADING_LEFT;
	if (onRight && dx > 0) return HEADING_RIGHT;
	if (onTop && dy < 0) return HEADING_UP;
	if (onBottom && dy > 0) return HEADING_DOWN;

	// Otherwise, choose based on target direction with preference for the side we're on
	const absX = Math.abs(dx);
	const absY = Math.abs(dy);

	// If the target is more horizontal
	if (absX > absY) {
		if (dx > 0) {
			// Target is to the right
			if (onRight) return HEADING_RIGHT;
			if (onTop) return HEADING_UP;
			if (onBottom) return HEADING_DOWN;
			return HEADING_RIGHT;
		} else {
			// Target is to the left
			if (onLeft) return HEADING_LEFT;
			if (onTop) return HEADING_UP;
			if (onBottom) return HEADING_DOWN;
			return HEADING_LEFT;
		}
