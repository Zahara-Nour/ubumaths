<script lang="ts">
	...
	import { renderRoughShape, renderRoughStroke } from '../core/rough-renderer';
	import {
		hitTestElements,
		getElementsInRect,
	...
	/** Reference to the group for sketch-mode shapes */
	let sketchShapesGroup: SVGGElement | null = $state(null);

	/** Reference to the group for sketch-mode strokes */
	let sketchStrokesGroup: SVGGElement | null = $state(null);
	...
	/** Perfect-style strokes (default rendering) */
	let perfectStrokes = $derived(
		strokeElements.filter((s) => !s.renderStyle || s.renderStyle === 'perfect')
	);

	/** Sketch-style strokes (roughjs rendering) */
	let sketchStrokes = $derived(strokeElements.filter((s) => s.renderStyle === 'sketch'));

	/** Only shape elements for rendering */
	let shapeElements = $derived(elements.filter((el): el is ShapeElement => el.type === 'shape'));

	/** Perfect-style shapes (default rendering) */
	let perfectShapes = $derived(
		shapeElements.filter((s) => !s.renderStyle || s.renderStyle === 'perfect')
	);

	/** Sketch-style shapes (roughjs rendering) */
	let sketchShapes = $derived(shapeElements.filter((s) => s.renderStyle === 'sketch'));
	...
	/**
	 * Render sketch-style shapes using roughjs
	 * Uses $effect to imperatively manage SVG nodes generated by roughjs
	 */
	$effect(() => {
		if (!sketchShapesGroup || !svgRef) return;

		// Clear previous content
		sketchShapesGroup.innerHTML = '';

		// Render each sketch shape
		for (const shape of sketchShapes) {
			...
			// Apply live position transform if dragging
			if (livePos || shapeRotation !== 0) {
			...
			sketchShapesGroup.appendChild(element);
		}
	});

	/**
	 * Render sketch-style strokes using roughjs
	 * Uses $effect to imperatively manage SVG nodes generated by roughjs
	 */
	$effect(() => {
		if (!sketchStrokesGroup || !svgRef) return;

		// Clear previous content
		sketchStrokesGroup.innerHTML = '';

		// Render each sketch stroke
		for (const stroke of sketchStrokes) {
			...
			const element = renderRoughStroke(svgRef, stroke);

			// Apply live position transform if dragging
			if (livePos || strokeRotation !== 0) {
				const transforms: string[] = [];
				if (livePos) {
					transforms.push(`translate(${livePos.dx}, ${livePos.dy})`);
				}
				if (strokeRotation !== 0) {
					transforms.push(`rotate(${strokeRotation}, ${strokeCenter.x}, ${strokeCenter.y})`);
				}
				element.setAttribute('transform', transforms.join(' '));
			}
	...
	function addStrokeElement() {
		const element: StrokeElement = {
			...
			renderStyle: toolState.renderStyle,
			roughSeed:
				toolState.renderStyle === 'sketch' ? Math.floor(Math.random() * 2147483647) : undefined
		};
	...
	function finalizeShape() {
		...
		if (currentTool === 'arrow') {
			...
			// Add renderStyle properties
			const shapeWithStyle = { ...shape, renderStyle, roughSeed, roughness };
			whiteboardStore.addElement(shapeWithStyle);
		}
		resetShapeState();
	}
...
<div class="whiteboard-canvas-container relative overflow-hidden bg-gray-100 {className}">
	...
	>
		...
		<g class="layer-content">
			<!-- Sketch-style strokes (rendered imperatively via $effect) -->
			<g class="sketch-strokes" bind:this={sketchStrokesGroup}></g>

			<!-- Perfect-style strokes (declarative SVG) -->
			{#each perfectStrokes as stroke (stroke.id)}
				{@const liveRot = whiteboardStore.liveRotations.get(stroke.id)}
				{@const strokeRotation = liveRot ?? stroke.rotation ?? 0}
			...
			<!-- Sketch-style shapes (rendered imperatively via $effect) -->
			<g class="sketch-shapes" bind:this={sketchShapesGroup}></g>

			<!-- Perfect-style shapes (declarative SVG) -->
			{#each perfectShapes as shape (shape.id)}
