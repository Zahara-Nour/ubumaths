		...
		Tables: {
			...
			python_file_assignments: {
				...
				Relationships: [
					...
					{
						foreignKeyName: 'python_file_assignments_file_id_fkey';
						columns: ['file_id'];
						isOneToOne: false;
						referencedRelation: 'python_files';
						referencedColumns: ['id'];
					}
				];
			};
			...
			tutor_messages: {
				Row: {
					cheat_detected: boolean | null;
					content: string;
					conversation_id: string;
					created_at: string | null;
					help_level: number | null;
					help_method_used: string | null;
					id: string;
...
export type Gender = 'boy' | 'girl' | 'female' | 'male';

// Notification Types (stored as strings in DB)
...
// Friendship Types
export type FriendshipType = 'friend' | 'best_friend' | 'accepted' | 'pending';
export interface FriendshipWithProfile extends Friendship {
	friend_profile?: {
		...
		role?: string;
		gender?: string | null;
		presence?: { is_online: boolean; last_seen: string } | boolean;
	};
}
