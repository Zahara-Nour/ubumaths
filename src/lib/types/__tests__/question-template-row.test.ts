 ...
 * la row pour la faire passer pour un `QuestionTemplate` : un statut inconnu
 * passait alors tel quel, et une variation absente devenait `undefined` là où
 * le générateur attend un tableau.
 */
const row = (overrides: Partial<QuestionTemplateRow> = {}): QuestionTemplateRow => ({
	id: 'modele-1',
...
describe('toQuestionTemplate', () => {
	it('conserve un modèle publié', () => {
		const modele = toQuestionTemplate(row());
	...
	it('retient un statut inconnu du côté brouillon', () => {
		...
		expect(toQuestionTemplate(row({ status: 'en_relecture' })).status).toBe('draft');
	});
	...
	it('rend un tableau vide quand les variations ne sont pas une liste', () => {
		...
		// variation ».
		expect(toQuestionTemplate(row({ variations: { statement: 'x' } })).variations).toEqual([]);
	});
