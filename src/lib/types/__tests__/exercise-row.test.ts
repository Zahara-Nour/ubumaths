 ...
 */
const row = (overrides: Partial<ExerciseRow> = {}): ExerciseRow => ({
	id: 'exo-1',
	slug: null,
	title: null,
	source: null,
...
describe('toExercise', () => {
	it('conserve un exercice complet', () => {
		const exercice = toExercise(row(), ['équations']);
	...
	it('ne fige pas une distribution inconnue', () => {
		...
		expect(toExercise(row({ distribution_mode: 'par_binome' })).distribution_mode).toBe(
			'on_demand'
		);
	});

	it('retombe sur automatisme devant une catégorie inconnue', () => {
		expect(toExercise(row({ category: 'defi_maison' })).category).toBe('automatisme');
	});

	it('écarte un niveau que l’application ne connaît pas', () => {
		expect(toExercise(row({ grades: ['3', 'terminale-euro'] })).grades).toEqual(['3']);
	});
	...
	it('rend les colonnes jsonb non listées comme absentes', () => {
		...
		const exercice = toExercise(row({ variables: { a: 1 }, variations: { x: 1 } }));
	...
	it('sans étiquette fournie, la liste est vide et non absente', () => {
		...
		expect(toExercise(row()).tags).toEqual([]);
	});
});
