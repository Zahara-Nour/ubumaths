...
const generatedStepsRationalInequality = z.object({
	kind: z.literal('rational-inequality'),
	inequality: z.string().min(1),
	options: generatedStepsOptionsSchema.optional()
});
...
const generatedStepsArithmeticFromBlank = z.object({
	kind: z.literal('arithmetic-from-blank'),
	expressionName: z.string().min(1),
	options: generatedStepsOptionsSchema.optional()
});

const generatedStepsLimit = z.object({
...
const generatedStepsRationalInequalityStrictZ = z
	.object({
		kind: z.literal('rational-inequality'),
		inequality: z.string().min(1),
		options: generatedStepsOptionsStrictZ.optional()
...
const generatedStepsArithmeticFromBlankStrictZ = z
	.object({
		kind: z.literal('arithmetic-from-blank'),
		expressionName: z.string().min(1),
		options: generatedStepsOptionsStrictZ.optional()
...
const generatedStepsLimitStrictZ = z
	.object({
		kind: z.literal('limit'),
		expression: z.string().min(1),
		variable: z.string().min(1).optional(),
