...
// Import from mathAST pedagogical layer (used by Mode B `generatedSteps`)
 ...
 *
 * All fields are optional. When omitted, the constraint defaults to 'warn'
 * (partial credit penalty). Use 'off' to disable a check, 'strict' to require it.
 */
...
export interface QuestionCorrection {
	...
	/** Mode A — author-written explanation steps (markdown templates) */
	steps?: TemplateMarkdown[];
	/**
	 * Mode B — declarative generated steps. When present, the pedagogical
	 * pipeline is invoked at instance-generation time and pre-renders
	 * `RenderedStep[]` into `ResolvedCorrection._renderedSteps`.
	 *
	 * Mode A and Mode B can coexist. Mode A wins at display time when both
	 * are populated (the author has explicitly authored steps).
	 */
...
export interface ResolvedCorrection {
	...
	/**
	 * Mode B — copied as-is from the template's `QuestionCorrection.generatedSteps`.
	 * The `expression` / `equation` strings still contain `{{vars}}` —
	 * resolution happens inside `generateCorrection()` using the instance's
	 * `resolvedVariables`.
	 */
	generatedSteps?: GeneratedSteps;
	/**
	 * Mode B — pre-rendered steps populated by `generateCorrection()`.
	 * Prefixed `_` to signify "computed, not authored". Absent when
	 * `generatedSteps` is not declared OR when generation failed silently.
	 */
	_renderedSteps?: readonly RenderedStep[];
}

// ============================================================================
// MODE B — GENERATED STEPS (declarative correction)
// ============================================================================

/**
 * Declarative description of a pedagogical correction to be generated by the
 * mathAST pipelines. The author writes a `kind` + an expression / equation
 * (with template variables); `generateCorrection()` resolves variables, picks
 * the matching pipeline, and stores `RenderedStep[]` on the instance.
 *
 * V1 supports two domains :
 *  - `arithmetic` — calls `pedagogical-arithmetic` (cf. {@link ./mathAST/pedagogical-arithmetic}).
 *  - `linear-equation` — calls `pedagogical-solve/linear` (cf. {@link ./mathAST/pedagogical-solve}).
 *
 * Future kinds (`solve`, `arithmetic-from-blank`, `differentiate`, …) plug in
 * by adding a discriminator and a corresponding case in the generator.
 */
export type GeneratedSteps =
	| {
			readonly kind: 'arithmetic';
			/** Template expression — supports `{{a}}`, `{{eval:...}}`, etc. */
...
/**
 * Knobs influencing the rendered steps. All optional with sensible defaults.
 */
export interface GeneratedStepsOptions {
	/**
	 * Target school level. `'auto'` (default) maps the template's `grades` to
	 * a `SchoolLevel` via `gradeLevelToSchoolLevel()`. Explicit values
	 * override the auto-derivation.
	 */
	readonly schoolLevel?: 'auto' | SchoolLevel;
	/**
	 * Verbosity passed to the renderer. `'detailed'` (default) emits
	 * explanation strings ; `'summarized'` keeps only titles + LaTeX.
	 */
