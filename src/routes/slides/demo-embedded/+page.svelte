<script lang="ts">
	...
	const sizePresets = [
		...
		{ name: 'Large', width: 1024, height: 576 },
		{ name: 'HD', width: 1280, height: 720 }
	];

	let selectedSize = $state(sizePresets[1]); // Default to Medium
...
<svelte:head>
	<title>UbuSlides Demo - Embedded</title>
</svelte:head>

<div class="page">
	<header>
		<h1>UbuSlides - Mode Embarque</h1>
		<p>Testez les slides dans differentes tailles de container</p>

		<div class="size-controls">
			{#each sizePresets as preset (preset.name)}
				<button class:active={selectedSize === preset} onclick={() => (selectedSize = preset)}>
		...
		<p class="info">
			Slide actuelle: {currentSlide.h + 1}{currentSlide.v > 0 ? `.${currentSlide.v + 1}` : ''}
			| Taille: {selectedSize.width}x{selectedSize.height}
		</p>
	</header>
	...
	<main>
		...
		>
			<Deck config={{ fullscreen: false, minScale: 0.5 }} onslidechanged={handleSlideChanged}>
				<!-- ============================================ -->
				<!-- INTRO -->
				<!-- ============================================ -->
				...
				<Slide background="#1a1a2e">
					<h2>Bienvenue</h2>
					<p>
						UbuSlides permet de creer des presentations interactives avec support mathematique
						integre.
					</p>
				</Slide>
				...
				<Slide background="#16213e">
					{#snippet vertical()}
						...
						<Slide>
							...
							<p>
								<span class="fragment slide-right" style="display: inline-block;"
									>slide-right,
								</span>
