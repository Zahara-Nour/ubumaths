<!--
  ...
  1. Google OAuth ("Lycée Voltaire" button) - CURRENTLY DISABLED via GOOGLE_LOGIN_ENABLED.
     The server infrastructure (googleSignIn action + /auth/callback) is kept intact so
     the button can be re-enabled by flipping the flag to true (and updating the domain
     label below) if a Google Workspace domain is used again.
...
<script lang="ts">
	...
	let showEmailForm = $state(!GOOGLE_LOGIN_ENABLED);
		...
		<Card.Content class="space-y-6">
			...
			{:else}
				...
				<div transition:slide={{ duration: 200 }}>
					...
					>
						{#if GOOGLE_LOGIN_ENABLED}
							<!-- "Retour" only makes sense when there is another method to go back to -->
