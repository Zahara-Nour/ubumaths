<script lang="ts">
	...
	// Archive message
	async function archiveMessage(messageId: string, event: MouseEvent) {
		event.stopPropagation();
		await privateMessages.updateStatus(messageId, 'archived');
	}
...
<div class="flex h-full flex-col">
	...
	<div class="border-b border-border bg-card p-4">
		<div class="flex items-center justify-between">
			<div>
				<h1 class="text-2xl font-bold text-foreground">Boîte de réception</h1>
	...
	<div class="flex-1 overflow-y-auto">
		...
		{:else if filteredMessages().length === 0}
			<div class="flex h-full flex-col items-center justify-center text-center">
				...
				{:else}
					<h3 class="text-lg font-semibold text-foreground">Aucun message</h3>
					<p class="mt-2 text-sm text-muted-foreground">Votre boîte de réception est vide</p>
				{/if}
			</div>
		{:else}
							...
							<div class="flex items-start justify-between gap-2">
								...
								>
									...
									<Button
										variant="ghost"
										size="sm"
										onclick={(e) => archiveMessage(message.message_id, e)}
										class="h-8 w-8 p-0"
									>
