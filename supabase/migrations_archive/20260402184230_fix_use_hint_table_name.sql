-- ============================================================================
-- Migration: Fix table name in use_hint()
-- Created: 2026-04-02
-- ============================================================================
-- BUG: gidouilles_history was renamed to gidouilles_activity in migration
--      20260103235000, but use_hint() (last updated in 20260304072306)
--      still referenced the old name, causing 42P01 errors when students
--      used hints with gidouilles.
-- ============================================================================
...
BEGIN
  ...
  IF v_vip_cards IS NOT NULL THEN
    ...
    ORDER BY
      -- 1. Already-started cards first (usesRemaining < uses_total)
      CASE
  ...
  -- FIX: use gidouilles_activity (renamed from gidouilles_history in 20260103235000)
  INSERT INTO public.gidouilles_activity (student_id, delta, reason, source_id)
...
COMMENT ON FUNCTION public.use_hint IS
  'Use a hint in Minesweeper. Smart card selection: prefers already-started hint cards, then fewest charges. Priority: 1) VIP hint card (via use_vip_card with minesweeper context), 2) Gidouilles (1.0 cost). VIP hints get reduced penalty. Logs to gidouilles_activity.';
