-- ============================================================================
-- Fix: execute_trade passes class_id to gidouilles_activity
-- Remove duplicate gidouilles logging from marketplace_trades trigger
-- ============================================================================
-- PROBLEM:
--   execute_trade inserts into gidouilles_activity with class_id = NULL.
--   The gidouilles_activity trigger creates reward_events with class_id = NULL
--   (invisible to teachers via RLS).
--   Meanwhile, log_marketplace_trades_to_events ALSO creates gidouilles
--   reward_events (with proper class_id), causing duplicates for students.
--
-- FIX:
--   1. execute_trade looks up class_id for both participants
--   2. log_marketplace_trades_to_events stops logging gidouilles
--      (already handled by gidouilles_activity -> reward_events chain)
-- ============================================================================


-- ============================================================================
-- 1. Update execute_trade to pass class_id in gidouilles_activity
-- ============================================================================
...
BEGIN
    ...
    -- Get names for history logging (use firstname + lastname, no username column exists)
    ...
    -- Get class_id for both participants (for gidouilles_activity audit trail)
    ...
    -- Log gidouilles activity with proper class_id
    IF v_initiator_gidouilles > 0 THEN
...
'Atomically execute a trade. Updates vip_cards and gidouilles directly. Logs to vip_cards_activity and gidouilles_activity with proper class_id.';


-- ============================================================================
-- 2. Drop marketplace_trades -> reward_events trigger and function
-- ============================================================================
-- Gidouilles are now properly logged via:
--   execute_trade -> gidouilles_activity (with class_id)
--                 -> trigger log_gidouilles_activity_to_events
--                 -> reward_events
--
-- VIP cards are handled via:
--   execute_trade -> vip_cards_activity
--                 -> trigger log_vip_cards_to_events
--                 -> reward_events
--
-- This trigger was creating DUPLICATE gidouilles reward_events.
