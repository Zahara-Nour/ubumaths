-- Smart hint card selection: prefer already-started cards, then minimum charges.
--
-- Previous behavior: picked the oldest card (ORDER BY earnedAt ASC).
-- New behavior:
--   1. Prefer a card already started (usesRemaining < template.uses_total)
--   2. Among non-started cards, pick the one with fewest charges (usesRemaining ASC)
--   This avoids spreading charges across multiple cards unnecessarily.
...
COMMENT ON FUNCTION public.use_hint IS
  'Use a hint in Minesweeper. Smart card selection: prefers already-started hint cards, then fewest charges. Priority: 1) VIP hint card (via use_vip_card with minesweeper context), 2) Gidouilles (1.0 cost). VIP hints get reduced penalty.';
