...
-- This allows searching "zoe" to find "Zoé" in the profiles table

CREATE EXTENSION IF NOT EXISTS unaccent;

-- Add a comment explaining the extension's purpose
COMMENT ON EXTENSION unaccent IS 'Removes accents from text for accent-insensitive search (e.g., "zoe" matches "Zoé")';

-- Create a function to search users with accent-insensitive matching
-- This function handles the unaccent logic that can't be done via the JS client
...
-- Grant execute permission to authenticated users (admins only via RLS/app logic)
