-- ============================================================================
-- Migration: Fix record_game_reward() missing class_id
-- Created: 2026-03-11
-- ============================================================================
-- BUG: record_game_reward() inserts class_id = NULL into gidouilles_activity.
--      The trigger log_gidouilles_activity_to_events propagates this NULL to
--      reward_events. The teacher RLS policy on reward_events requires
--      class_id IS NOT NULL, so game reward events are invisible to teachers
--      in the journal/audit trail.
--
-- FIX: Look up the student's active class_id and use it instead of NULL.
--      Also backfill existing reward_events that have NULL class_id.
-- ============================================================================
...
'Records a game victory and awards gidouilles. Enforces 1 gidouille/day limit. Includes class_id for teacher audit trail visibility.';

-- ============================================================================
-- Backfill: Update existing reward_events with NULL class_id
-- for daily_game_reward entries
-- ============================================================================
...
WHERE re.class_id IS NULL
  AND re.source_table = 'gidouilles_activity'
  AND re.description LIKE '%daily_game_reward%';

-- Also backfill gidouilles_activity entries with NULL class_id
UPDATE public.gidouilles_activity ga
SET class_id = (
  SELECT cm.class_id
  FROM public.class_members cm
  WHERE cm.student_id = ga.student_id
    AND cm.status = 'active'
  LIMIT 1
)
WHERE ga.class_id IS NULL
  AND ga.reason LIKE 'daily_game_reward:%';
