-- Remove game-specific validation from use_vip_card RPC.
-- The RPC keeps the context match check (p_context == activation_context)
-- but no longer queries minesweeper_games directly.
-- Game-specific conditions (game in progress, bomb revealed, etc.)
-- are the responsibility of the calling game RPC (use_hint, use_minesweeper_undo).
...
BEGIN
  ...
  -- ========================================================================
  -- VALIDATE ACTIVATION CONTEXT (match only, no game-specific checks)
  -- ========================================================================
  -- If the template requires an activation_context, the caller must provide
  -- the matching context string. Game-specific conditions (game in progress,
  -- bomb revealed, etc.) are validated by the calling game RPC.

  IF v_template.activation_context IS NOT NULL AND v_template.activation_context != 'any' THEN
    IF p_context IS NULL OR p_context != v_template.activation_context THEN
      RETURN jsonb_build_object(
        'success', FALSE,
        'error', 'This card requires context: ' || v_template.activation_context
      );
    END IF;
  END IF;
...
Context check: verifies p_context matches template.activation_context (if set).
