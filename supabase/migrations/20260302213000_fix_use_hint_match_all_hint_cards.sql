-- Fix use_hint RPC to match all minesweeper hint card variants
-- (minesweeper-hint, minesweeper-hint-2, minesweeper-hint-3)
-- Previously only matched 'minesweeper-hint' exactly.
...
BEGIN
  ...
  -- Match all hint variants: minesweeper-hint, minesweeper-hint-2, minesweeper-hint-3
...
COMMENT ON FUNCTION public.use_hint IS
  'Use a hint in Minesweeper. Matches all hint card variants (minesweeper-hint*). Priority: 1) VIP hint card (via use_vip_card with minesweeper context), 2) Gidouilles (1.0 cost). VIP hints get reduced penalty.';
