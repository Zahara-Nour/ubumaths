-- Fix google_course_id in google_classroom_coursework table (v2 - fixed casting)
--
...
-- Match by casting both sides to text for comparison
INNER JOIN google_classroom_courses c ON c.id::text = cw.google_course_id::text
WHERE
    -- Only fix records where google_course_id looks like a UUID
    cw.google_course_id::text ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$';
