-- ============================================================================
-- Ré-amorçage du référentiel de 1ʳᵉ spécialité — formules en ubumark
-- ============================================================================
-- Les 41 libellés qui portaient des maths les écrivaient en Unicode entre
-- backticks (`uₙ`, `∀`, `‖u⃗ + v⃗‖²`) : lisible dans un fichier, illisible dans
-- l'application. Ils passent en LaTeX `$…$` (syntaxe ubumark, rendu MathLive).
--
-- ⚠️ DESTRUCTIF sur le niveau `1_SPE` : la purge ci-dessous supprime les thèmes,
-- donc en cascade les objectifs et les points, donc leurs codes.
--
-- Sûr UNIQUEMENT tant que rien n'est accroché aux points. Vérifié avant écriture
-- (2026-08-29, prod comme local) : 0 tag de question, 0 tag d'exercice,
-- 0 couverture de cahier de texte, 0 acquisition élève, 0 liste d'automatismes.
-- Cette migration ne doit PAS être rejouée une fois la banque de questions
-- taguée — elle effacerait ces liens sans avertissement.
--
-- Le corps est celui du seed généré : la garde `IF EXISTS … RETURN` reste, mais
-- elle passe toujours ici puisque la purge vient de vider le niveau.
-- ============================================================================
...
FROM (VALUES
	...
	('1SPE-002', 'Vocabulaire ensembliste et logique', 'Ensembles', 'Symboles de base correspondants : $\varnothing$, $\in$, $\subset$, $\cap$, $\cup$, $\{\,\dots\,\}$', 2, 'connaissance', 'attendu'),
	...
	('1SPE-005', 'Vocabulaire ensembliste et logique', 'Ensembles', 'Notation du complémentaire d''un sous-ensemble $A$ de $E$ : $\bar{A}$ (notation des probabilités) ou $E \setminus A$', 5, 'connaissance', 'attendu'),
	('1SPE-006', 'Vocabulaire ensembliste et logique', 'Ensembles', 'Notation $\operatorname{Card}(A)$ pour le cardinal d''un ensemble fini', 6, 'connaissance', 'attendu'),
	...
	('1SPE-013', 'Vocabulaire ensembliste et logique', 'Logique et raisonnement', 'Utiliser les quantificateurs (les symboles $\forall$ et $\exists$ ne sont pas exigibles) et repérer les quantifications implicites, particulièrement dans les propositions conditionnelles', 7, 'savoir_faire', 'attendu'),
	...
	('1SPE-023', 'Algèbre', 'Suites numériques, modèles discrets', 'Exemples de modes de génération d''une suite : explicite $u_n = f(n)$, par une relation de récurrence $u_{n+1} = f(u_n)$, par un algorithme, par des motifs géométriques', 1, 'connaissance', 'attendu'),
	('1SPE-024', 'Algèbre', 'Suites numériques, modèles discrets', 'Notations : $u(n)$, $u_n$, $(u(n))$, $(u_n)$', 2, 'connaissance', 'attendu'),
	('1SPE-025', 'Algèbre', 'Suites numériques, modèles discrets', 'Suites arithmétiques : exemples, définition, calcul du terme général ; lien avec l''étude d''évolutions successives à accroissements constants ; lien avec les fonctions affines ; calcul de $1 + 2 + \dots + n$', 3, 'connaissance', 'attendu'),
	('1SPE-026', 'Algèbre', 'Suites numériques, modèles discrets', 'Suites géométriques : exemples, définition, calcul du terme général ; lien avec l''étude d''évolutions successives à taux constant ; lien avec la fonction exponentielle ; calcul de $1 + q + \dots + q^n$', 4, 'connaissance', 'attendu'),
	...
	('1SPE-037', 'Algèbre', 'Suites numériques, modèles discrets', 'Calcul de $1 + 2 + \dots + n$', 15, 'demonstration', 'attendu'),
	('1SPE-038', 'Algèbre', 'Suites numériques, modèles discrets', 'Calcul de $1 + q + \dots + q^n$', 16, 'demonstration', 'attendu'),
	...
	('1SPE-053', 'Algèbre', 'Équations, fonctions polynômes du second degré', 'Factorisation de $x^n - 1$ par $x - 1$, de $x^n - a^n$ par $x - a$', 9, 'savoir_faire', 'approfondissement'),
	('1SPE-054', 'Algèbre', 'Équations, fonctions polynômes du second degré', 'Déterminer deux nombres réels connaissant leur somme $s$ et leur produit $p$ comme racines de la fonction polynôme $x \mapsto x^2 - sx + p$', 10, 'savoir_faire', 'approfondissement'),
	...
	('1SPE-058', 'Analyse', 'Dérivation', 'Approximation linéaire : fonction affine tangente $x \mapsto f(a) + f''(a)(x - a)$ et approximation de $f(a + h)$ par $f(a) + f''(a)h$', 4, 'connaissance', 'attendu'),
	...
	('1SPE-062', 'Analyse', 'Dérivation', 'Pour $n$ dans $\mathbb{Z}$, fonction dérivée de la fonction $x \mapsto x^n$', 8, 'connaissance', 'attendu'),
	...
	('1SPE-082', 'Analyse', 'Variations et courbes représentatives des fonctions', 'Étudier, en lien avec la dérivation, une fonction polynôme du second degré : variations, extrémum, allure selon le signe du coefficient de $x^2$', 7, 'savoir_faire', 'attendu'),
	...
	('1SPE-084', 'Analyse', 'Fonction exponentielle', 'Définition de la fonction exponentielle comme unique fonction dérivable sur $\mathbb{R}$ vérifiant $f'' = f$ et $f(0) = 1$ (existence et unicité admises) ; notation $\exp(x)$', 1, 'connaissance', 'attendu'),
	('1SPE-085', 'Analyse', 'Fonction exponentielle', 'Pour tous réels $x$ et $y$, $\exp(x + y) = \exp(x)\exp(y)$ et $\exp(x)\exp(-x) = 1$ ; nombre $e$ ; notation $e^x$', 2, 'connaissance', 'attendu'),
	...
	('1SPE-088', 'Analyse', 'Fonction exponentielle', 'Pour $a$ réel, dérivée de la fonction $t \mapsto e^{at}$', 5, 'savoir_faire', 'attendu'),
	('1SPE-089', 'Analyse', 'Fonction exponentielle', 'Pour une valeur numérique strictement positive de $k$, représenter graphiquement les fonctions $t \mapsto e^{-kt}$ et $t \mapsto e^{kt}$', 6, 'savoir_faire', 'attendu'),
	...
	('1SPE-092', 'Analyse', 'Fonction exponentielle', 'Détermination d''une valeur approchée de $e$ à l''aide de la suite $\left(\left(1 + \tfrac{1}{n}\right)^n\right)$', 9, 'savoir_faire', 'approfondissement'),
	('1SPE-093', 'Analyse', 'Fonction exponentielle', 'Unicité d''une fonction $f$ dérivable sur $\mathbb{R}$ vérifiant $f'' = f$ et $f(0) = 1$', 10, 'savoir_faire', 'approfondissement'),
	...
	('1SPE-101', 'Analyse', 'Trigonométrie', 'Calcul de $\cos\left(\tfrac{\pi}{4}\right)$, $\sin\left(\tfrac{\pi}{4}\right)$, $\cos\left(\tfrac{\pi}{3}\right)$, $\sin\left(\tfrac{\pi}{3}\right)$', 6, 'demonstration', 'attendu'),
	('1SPE-102', 'Analyse', 'Trigonométrie', 'Approximation de $\pi$ par la méthode d''Archimède', 7, 'savoir_faire', 'approfondissement'),
	...
	('1SPE-105', 'Géométrie', 'Calcul vectoriel et produit scalaire', 'Développement de $\|\vec{u} + \vec{v}\|^2$ et $\|\vec{u} - \vec{v}\|^2$ ; formule d''Al-Kashi', 3, 'connaissance', 'attendu'),
	('1SPE-106', 'Géométrie', 'Calcul vectoriel et produit scalaire', 'Transformation de l''expression $\overrightarrow{MA} \cdot \overrightarrow{MB}$', 4, 'connaissance', 'attendu'),
	...
	('1SPE-111', 'Géométrie', 'Calcul vectoriel et produit scalaire', 'Ensemble des points $M$ tels que $\overrightarrow{MA} \cdot \overrightarrow{MB} = 0$ (démonstration avec le produit scalaire)', 9, 'demonstration', 'attendu'),
	...
	('1SPE-124', 'Géométrie', 'Géométrie repérée', 'Déterminer l''intersection d''un cercle ou d''une parabole d''équation $y = ax^2 + bx + c$ avec une droite parallèle à un axe', 10, 'savoir_faire', 'approfondissement'),
	...
	('1SPE-128', 'Probabilités et statistiques', 'Probabilités conditionnelles et indépendance', 'Pour $n \leqslant 4$, répétition de $n$ épreuves de Bernoulli indépendantes et identiques', 4, 'connaissance', 'attendu'),
	...
	('1SPE-132', 'Probabilités et statistiques', 'Probabilités conditionnelles et indépendance', 'Pour $n \leqslant 4$, représenter l''arbre associé à la répétition de $n$ épreuves de Bernoulli indépendantes et identiques afin de calculer des probabilités', 8, 'savoir_faire', 'attendu'),
	('1SPE-133', 'Probabilités et statistiques', 'Probabilités conditionnelles et indépendance', 'Méthode de Monte-Carlo : estimation de l''aire sous la parabole, estimation du nombre $\pi$', 9, 'savoir_faire', 'approfondissement'),
	...
	('1SPE-141', 'Probabilités et statistiques', 'Variables aléatoires réelles', 'Interpréter en situation et utiliser les notations $\{X = a\}$, $\{X \leqslant a\}$, $P(X = a)$, $P(X \leqslant a)$', 6, 'savoir_faire', 'attendu'),
	...
	('1SPE-149', 'Probabilités et statistiques', 'Variables aléatoires réelles', 'Pour $X$ variable aléatoire, étude de la fonction du second degré $x \mapsto E\big((X - x)^2\big)$', 14, 'savoir_faire', 'approfondissement'),
