-- ============================================================================
-- RPC: Reorder worksheet exercises atomically
-- ============================================================================
-- This function updates exercise positions in a single transaction,
-- avoiding unique constraint conflicts on (worksheet_id, section_id, position)

CREATE OR REPLACE FUNCTION reorder_worksheet_exercises(
  p_worksheet_id UUID,
  p_exercises JSONB  -- Array of {id: uuid, position: int, section_id: uuid|null}
)
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION reorder_worksheet_exercises(UUID, JSONB) TO authenticated;

COMMENT ON FUNCTION reorder_worksheet_exercises IS 'Atomically reorder worksheet exercises, avoiding unique constraint conflicts';
