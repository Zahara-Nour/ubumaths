-- Migration: Save Google OAuth avatar_url to profile
-- Created: 2025-10-15
-- Purpose: Update handle_new_user() to extract and save Google profile picture URL

-- Replace the handle_new_user function with updated version that saves avatar_url
...
BEGIN
  ...
  -- Google OAuth stores avatar in 'picture' field, but check both
...
EXCEPTION
  WHEN unique_violation THEN
    ...
    IF google_avatar IS NOT NULL THEN
      ...
      RAISE NOTICE 'Updated avatar for existing user: %', NEW.email;
    END IF;
    RETURN NEW;
  WHEN OTHERS THEN
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Automatically creates a profile when a user signs up. Extracts and saves Google OAuth avatar_url if available. If the user email exists in pending_students, uses pre-populated data and enrolls them in pre-assigned classes. Otherwise creates a default student profile. Updates avatar for existing profiles if not already set.';
