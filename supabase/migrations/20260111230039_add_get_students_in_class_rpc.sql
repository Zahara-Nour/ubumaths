-- Migration: Add RPC function to get students in a class by grade
-- This function bypasses RLS to allow students to see students in other classes
-- of the same grade level for the friend-adding feature
...
GRANT EXECUTE ON FUNCTION public.get_students_in_class_by_grade(uuid) TO authenticated;

COMMENT ON FUNCTION public.get_students_in_class_by_grade(uuid) IS
  'Returns students in a class if it matches the current user''s grade level. Used for friend-adding feature.';
