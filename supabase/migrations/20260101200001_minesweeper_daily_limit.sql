-- ============================================================================
-- Migration: Update complete_minesweeper_game() for Daily Limit System
-- Purpose: Integrate with daily_game_rewards for 1 gidouille/day limit
-- Date: 2026-01-01
-- ============================================================================
--
-- CHANGES:
-- 1. Remove daily_mult from final calculation
-- 2. Calculate theoretical_reward (before daily limit)
-- 3. Call record_game_reward() to get actual reward (0 or 1)
-- 4. Store actual_reward in minesweeper_games.gidouilles_awarded
-- 5. Add to breakdown: theoretical_reward, actual_reward, is_first_win_of_day, week_best_reward
--
-- FORMULA CHANGE:
--   OLD: gidouilles = base × time_mult × (1 - hint_penalty) × daily_mult
--   NEW: theoretical_reward = base × time_mult × (1 - hint_penalty)
...
BEGIN
  ...
  -- ========================================================================
  -- NEW: Calculate theoretical reward (WITHOUT daily_mult)
  -- ========================================================================
  ...
  -- ========================================================================
  -- NEW: Call record_game_reward to get actual reward (0 or 1)
  -- ========================================================================
  ...
  v_breakdown := jsonb_build_object(
    ...
    -- NEW: Daily limit fields (replacing wins_today and daily_mult)
  ...
  SET
    ...
    completed_at = NOW(),
    time_seconds = v_time_seconds,
    gidouilles_awarded = v_actual_reward,  -- Changed: actual_reward (0 or 1)
...
COMMENT ON FUNCTION public.complete_minesweeper_game IS
  'Completes Minesweeper game with daily limit system.
   ...
   Returns:
     - gidouilles_earned: Actual reward (0 or 1)
     - achievements: Unlocked achievements
     - points_earned: Points for leaderboard
     - breakdown: Detailed calculation factors including:
         - cycle: Pedagogical cycle (null if no grade)
         - reference_time: Dynamic time for cycle/difficulty
         - time_mult, hint_penalty: Multipliers used
         - theoretical_reward: What you would have earned without daily limit
         - actual_reward: What you actually earned (0 or 1)
         - is_first_win_of_day: Whether this was your first win today
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Migration Complete: Minesweeper Daily Limit';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Updated: complete_minesweeper_game() function';
  RAISE NOTICE '';
  RAISE NOTICE 'CHANGES:';
  RAISE NOTICE '  - Removed daily_mult from calculation';
  RAISE NOTICE '  - Added theoretical_reward (base x time x hints)';
  RAISE NOTICE '  - Calls record_game_reward() for daily limit';
  RAISE NOTICE '  - Breakdown now includes:';
  RAISE NOTICE '    - theoretical_reward';
  RAISE NOTICE '    - actual_reward (0 or 1)';
  RAISE NOTICE '    - is_first_win_of_day';
  RAISE NOTICE '    - week_best_reward';
  RAISE NOTICE '========================================';
END;
$$ LANGUAGE plpgsql;
