-- Migration: Add Approval Check to draw_multiple_vip_cards Function
-- Created: 2025-11-09
--
-- This migration updates the draw_multiple_vip_cards() function to require
-- teacher approval before allowing students to use VIP cards with draw_cards actions.
--
-- CHANGES:
-- --------
-- - Add v_card_activation_requested_at and v_card_activation_approved_at variables
-- - Check if activation was requested and require approval before execution
-- - Clear activation metadata fields after card is marked as used
--
-- FLOW:
-- -----
...
DECLARE
  ...
  v_card_used_at TEXT;
  v_card_activation_requested_at TEXT;
  v_card_activation_approved_at TEXT;
...
BEGIN
  ...
  ELSIF p_payment_method = 'vip_card' THEN
    ...
    -- Check if activation was requested (student flow), require approval
    v_card_activation_requested_at := v_card_instance->>'activationRequestedAt';
    v_card_activation_approved_at := v_card_instance->>'activationApprovedAt';

    IF v_card_activation_requested_at IS NOT NULL AND v_card_activation_approved_at IS NULL THEN
      RAISE EXCEPTION 'This card activation has not been approved yet. Please wait for teacher approval.';
    END IF;
    ...
    -- Clear activation fields since card is now used
    v_vip_cards := v_vip_cards #- ARRAY[p_vip_card_instance_id::text, 'activationRequestedAt'];
    v_vip_cards := v_vip_cards #- ARRAY[p_vip_card_instance_id::text, 'activationRequestedBy'];
    v_vip_cards := v_vip_cards #- ARRAY[p_vip_card_instance_id::text, 'activationApprovedAt'];
    v_vip_cards := v_vip_cards #- ARRAY[p_vip_card_instance_id::text, 'activationApprovedBy'];
