-- ============================================================================
-- Ré-amorçage 1ʳᵉ spé — commandes LaTeX inconnues de MathLive
-- ============================================================================
-- Deux commandes du référentiel ne sont pas dans la table de MathLive et
-- s'affichaient donc en clair au milieu des libellés, à l'écran :
--
--   \dots            → \cdots entre opérateurs (1 + 2 + \cdots + n)
--                       \ldots dans un ensemble ({ \ldots })
--   \overrightarrow  → \vec
--
-- Le choix entre \cdots et \ldots est typographique : points centrés entre
-- opérateurs binaires, points bas dans une énumération.
--
-- `\vec{MA}` au lieu de `\overrightarrow{MA}` s'écarte de la convention
-- française pour un vecteur défini par deux points, mais MathLive ne connaît
-- pas `\overrightarrow` : une flèche courte vaut mieux qu'un libellé qui
-- affiche « \overrightarrow{MA} » en toutes lettres à l'élève.
--
-- Le générateur refuse désormais d'écrire si une commande LaTeX est inconnue de
-- MathLive, en nommant les points fautifs. Il lit la table du paquet plutôt
-- qu'une copie figée, pour suivre ses montées de version.
--
-- ⚠️ DESTRUCTIF sur `1_SPE` (purge puis ré-amorçage). Sûr tant que rien n'est
-- accroché aux points — vérifié avant écriture.
-- ============================================================================
...
FROM (VALUES
	...
	('1SPE-002', 'Vocabulaire ensembliste et logique', 'Ensembles', 'Symboles de base correspondants : $\varnothing$, $\in$, $\subset$, $\cap$, $\cup$, $\{\,\ldots\,\}$. Notation du complémentaire d''un sous-ensemble $A$ de $E$ : $\bar{A}$ (notation des probabilités) ou $E \setminus A$', 2, 'connaissance', 'attendu'),
	...
	('1SPE-026', 'Algèbre', 'Suites numériques, modèles discrets', 'Suites arithmétiques : calcul de $1 + 2 + \cdots + n$', 5, 'connaissance', 'attendu'),
	...
	('1SPE-029', 'Algèbre', 'Suites numériques, modèles discrets', 'Suites géométriques : calcul de $1 + q + \cdots + q^n$', 8, 'connaissance', 'attendu'),
	...
	('1SPE-042', 'Algèbre', 'Suites numériques, modèles discrets', 'Calcul de $1 + 2 + \cdots + n$', 21, 'demonstration', 'attendu'),
	('1SPE-043', 'Algèbre', 'Suites numériques, modèles discrets', 'Calcul de $1 + q + \cdots + q^n$', 22, 'demonstration', 'attendu'),
	...
	('1SPE-124', 'Géométrie', 'Calcul vectoriel et produit scalaire', 'Transformation de l''expression $\vec{MA} \cdot \vec{MB}$', 8, 'connaissance', 'attendu'),
	...
	('1SPE-131', 'Géométrie', 'Calcul vectoriel et produit scalaire', 'Ensemble des points $M$ tels que $\vec{MA} \cdot \vec{MB} = 0$ (démonstration avec le produit scalaire)', 15, 'demonstration', 'attendu'),
