-- Drop the teacher VIP-card override layer (mono-teacher cleanup, "B pur")
-- =============================================================================
-- Mono-teacher: `teacher_vip_card_overrides` (per-teacher subtractive override)
-- is redundant with the global flag `vip_card_templates.is_enabled`. A card is
-- now drawable iff is_enabled = TRUE. We remove the override table, its dead
-- companion function, and the two override clauses inside the draw RPC.
--
-- No data fold needed: prod has zero override rows with is_enabled = FALSE (the
-- single row, candy=TRUE, was a no-op since candy is globally disabled), so the
-- effective draw set is unchanged.

BEGIN;

-- ===== 1. draw_multiple_vip_cards: remove the two override (NOT EXISTS) clauses =====
...
-- ===== 2. Drop the dead companion function (0 app callers; referenced the table) =====
DROP FUNCTION IF EXISTS "public"."get_available_cards_for_student"("uuid");

-- ===== 3. Drop the override table (its 6 RLS policies are dropped with it) =====
DROP TABLE IF EXISTS "public"."teacher_vip_card_overrides";
