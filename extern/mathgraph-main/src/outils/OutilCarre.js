...
export default OutilCarre
/**
 * Outil servant à créer un carré direct en cliquant sur deux points
 * @param {MtgApp} app L'application propriétaire
 * @constructor
 */
function OutilCarre (app) {
  OutilPar2Pts.call(this, app, 'Carre', 32990, true)
}

OutilCarre.prototype = new OutilPar2Pts()

OutilCarre.prototype.creeObjet = function () {
  const app = this.app
  const li = app.listePr
  const indiceImpInitial = li.longueur()
  // On charge la construction qui est contenue dans this.app.docConst
  const proto = this.app.docCons.getPrototype('CarreDirect')
...
OutilCarre.prototype.ajouteObjetsVisuels = function () {
  const app = this.app
  ...
  const proto = app.docCons.getPrototype('CarreDirect')
 ...
 */
OutilCarre.prototype.creationPointPossible = function () {
  return true
}

OutilCarre.prototype.isPointTool = function () {
  return true
}

OutilCarre.prototype.isLineTool = function () {
  return true
}
