...
export default OutilPunaiser
/**
 * Outil servant à marquer un point pour la trace
 * @param {MtgApp} app L'application propriétaire
 * @constructor
 */
function OutilPunaiser (app) {
  Outil.call(this, app, 'Punaiser', 32955, true)
}

OutilPunaiser.prototype = new Outil()

OutilPunaiser.prototype.select = function () {
  ...
  // app.listeClignotante.ajoutePointsLibresVisibles(app.listePr, true)
  app.listeClignotante.addVisibleObjetsFixedOrNot(app.listePr, NatObj.NPointMobile, false)
  ...
  app.indication('indPun')
}

OutilPunaiser.prototype.deselect = function () {
...
OutilPunaiser.prototype.traiteObjetDesigne = function (elg) {
  const app = this.app
  elg.fixed = true
  toast({ title: 'ObPunaise' })
  // S'il n'y a plus rien à punaiser on active l'outil de capture
  if (app.listeClignotante.nbVisibleObjectsFixedOrNot(NatObj.NPointMobile, false, true) === 0) {
    this.deselect()
    addQueue(() => {
...
OutilPunaiser.prototype.activationValide = function () {
  return this.app.listePr.nbVisibleObjectsFixedOrNot(NatObj.NPointMobile, false) > 0
}
